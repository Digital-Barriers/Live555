...
void Groupsock::changeDestinationParameters(struct in_addr const &newDestAddr,
  ...
  if (dest == NULL) { // There's no existing 'destRecord' for this "sessionId";
                      // add a new one:
  ...
  // "dest" is an existing 'destRecord' for this "sessionId"; change its values
  // to the new ones:
...
Boolean Groupsock::output(UsageEnvironment &env, unsigned char *buffer,
  ...
  do {
    ...
    if (!members().IsEmpty()) {
      numMembers =
          outputToAllMembersExcept(interfaceNotToFwdBackTo, ttl(), buffer,
                                   bufferSize, ourIPAddress(env));
      if (numMembers < 0)
        break;
    }
...
int Groupsock::outputToAllMembersExcept(DirectedNetInterface *exceptInterface,
                                        u_int8_t ttlToFwd, unsigned char *data,
                                        unsigned size,
...
Groupsock *GroupsockLookupTable::Fetch(UsageEnvironment &env,
                                       netAddressBits groupAddress, Port port,
