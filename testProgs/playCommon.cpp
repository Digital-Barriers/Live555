...
int main(int argc, char **argv) {
  ...
  while (argc > 1) {
    ...
    case 'K': { // Send periodic 'keep-alive' requests to keep broken server
                // sessions alive
    ...
    case 'E': {
      // specify initial absolute seek END time (trick play), using a string of
      // the form "YYYYMMDDTHHMMSSZ" or "YYYYMMDDTHHMMSS.<frac>Z"
      ...
      if (!subsession->initiate(simpleRTPoffsetArg)) {
        ...
        *env << ")\n";
        ...
        if (subsession->rtpSource() != NULL) {
          ...
          // if we were explicitly asked (using -B), or if the desired FileSink
          // buffer size happens to be larger than the current OS socket buffer
          // size. (The latter case is a heuristic, on the assumption that if
          // the user asked for a large FileSink buffer size, then the input
          // data rate may be large enough to justify increasing the OS socket
          // buffer size also.)
          ...
          if (socketInputBufferSize > 0 || fileSinkBufferSize > curBufferSize) {
            ...
            if (socketInputBufferSize >
                0) { // The user explicitly asked for the new socket buffer
                     // size; announce it:
              *env << "Changed socket receive buffer size for the \""
                   << subsession->mediumName() << "/" << subsession->codecName()
                   << "\" subsession from " << curBufferSize << " to "
                   << newBufferSize << " bytes\n";
            }
          }
        }
  ...
  if (outputQuickTimeFile || outputAVIFile) {
    ...
    while ((subsession = iter.next()) != NULL) {
      ...
      if (createOggFileSink) {
        ...
        // Normal case:
        fileSink = FileSink::createNew(*env, outFileName, fileSinkBufferSize,
                                       oneFilePerFrame);
      }
...
void subsessionByeHandler(void *clientData, char const *reason) {
  ...
  *env << " on \"" << subsession->mediumName() << "/" << subsession->codecName()
...
public:
  qosMeasurementRecord(struct timeval const &startTime, RTPSource *src)
      : fSource(src), fNext(NULL), kbits_per_second_min(1e20),
        kbits_per_second_max(0), kBytesTotal(0.0),
