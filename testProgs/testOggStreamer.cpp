...
void onOggFileCreation(OggFile *newFile, void *clientData) {
  ...
  for (unsigned i = 0; i < numTracks; ++i) {
    ...
    if (trackState[i].source != NULL) {
      ...
      if (trackState[i].sink != NULL) {
        ...
        // Having set up a track for streaming, add it to our RTSP server's
        // "ServerMediaSession":
  ...
  if (sms->numSubsessions() == 0) {
    ...
    *env << "(Perhaps the file does not exist, is not an 'Ogg' file, or has no "
            "tracks that we know how to stream.)\n";
    exit(1);
  }
  ...
  *env << "Beginning to read from file...\n";
  ...
  for (unsigned i = 0; i < numTracks; ++i) {
    if (trackState[i].sink != NULL && trackState[i].source != NULL) {
      trackState[i].sink->startPlaying(*trackState[i].source, afterPlaying,
                                       NULL);
    }
  }
}
