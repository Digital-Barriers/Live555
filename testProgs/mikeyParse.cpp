...
Boolean parseMikeyUnknown(u_int8_t const *& /*ptr*/,
...
Boolean parseMikeyHDR(u_int8_t const *&ptr, u_int8_t const *endPtr,
  ...
  for (u_int8_t i = 1; i <= numCryptoSessions; ++i) {
    fprintf(stderr, "\tPolicy_no_%d: %d;\tSSRC_%d: 0x%08x; ROC_%d: 0x%08x\n", i,
            getByte(ptr), i, get4Bytes(ptr), i, get4Bytes(ptr));
  }
...
static Boolean parseKeyDataSubPayload(u_int8_t const *&ptr,
  ...
  fprintf(stderr, "\t\tType: %d (%s)\n", Type,
          Type == 0
              ...
              : Type == 1
                    ? "TGK+SALT"
                    : Type == 2 ? "TEK" : Type == 3 ? "TEK+SALT" : "unknown");
  if (Type > 3)
    return False;
  ...
  fprintf(stderr, "\t\tKey Validity: %d (%s)\n", KV,
          KV == 0 ? "NULL"
                  : KV == 1 ? "SPI/MKI" : KV == 2 ? "Interval" : "unknown");
  Boolean hasKV = KV != 0;
  ...
  while (ptr < endPtr) {
    ...
    case 0: { // Encryption algorithm
      fprintf(stderr, " (%s)",
              ppVal == 0
                  ? "NULL"
                  : ppVal == 1 ? "AES-CM" : ppVal == 2 ? "AES-F8" : "unknown");
      break;
    }
