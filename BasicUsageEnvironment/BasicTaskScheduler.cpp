...
void BasicTaskScheduler::SingleStep(unsigned maxDelayTime) {
  ...
  if (selectResult < 0) {
    ...
    if (errno != EINTR && errno != EAGAIN) {
      ...
      // Because this failure is often "Bad file descriptor" - which is caused
      // by an invalid socket number (i.e., a socket number that had already
      // been closed) being used in "select()" - we print out the sockets that
      // were being used in "select()", to assist in debugging:
