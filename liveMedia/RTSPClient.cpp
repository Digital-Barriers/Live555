...
unsigned RTSPClient::sendSetupCommand(MediaSubsession &subsession,
  ...
  // However, if we're using a TLS connection, streaming over TCP doesn't work,
  // so disable it:
  if (fTLS.isNeeded)
  ...
  if (forceMulticastOnUnspecified)
    booleanFlags |= 0x4;
  return sendRequest(new RequestRecord(++fCSeq, "SETUP", responseHandler, NULL,
...
unsigned RTSPClient::sendPauseCommand(MediaSubsession &subsession,
  ...
  if (fCurrentAuthenticator < authenticator)
    fCurrentAuthenticator = *authenticator;
  return sendRequest(
      new RequestRecord(++fCSeq, "PAUSE", responseHandler, NULL, &subsession));
}
...
unsigned RTSPClient::sendRecordCommand(MediaSubsession &subsession,
  ...
  if (fCurrentAuthenticator < authenticator)
    fCurrentAuthenticator = *authenticator;
  return sendRequest(
      new RequestRecord(++fCSeq, "RECORD", responseHandler, NULL, &subsession));
}
...
void RTSPClient::setSpeed(MediaSession &session, float speed) {
  ...
  // The user should call this function after the MediaSession is instantiated,
...
RTSPClient::RTSPClient(UsageEnvironment &env, char const *rtspURL,
    ...
    : Medium(env), desiredMaxIncomingPacketSize(0),
      fVerbosityLevel(verbosityLevel), fCSeq(1),
      fAllowBasicAuthentication(True), fServerAddress(0),
      fTunnelOverHTTPPortNum(tunnelOverHTTPPortNum), fUserAgentHeaderStr(NULL),
      fUserAgentHeaderStrLen(0), fInputSocketNum(-1), fOutputSocketNum(-1),
      fBaseURL(NULL), fTCPStreamIdCount(0), fLastSessionId(NULL),
      fSessionTimeoutParameter(0), fSessionCookieCounter(0),
      fHTTPTunnelingConnectionIsPending(False), fTLS(*this) {
  setBaseURL(rtspURL);
...
unsigned RTSPClient::sendRequest(RequestRecord *request) {
  ...
  do {
    ...
    char const *const cmdFmt = "%s %s %s\r\n"
                               ...
                               "%s";
    unsigned cmdSize =
        strlen(cmdFmt) + strlen(request->commandName()) + strlen(cmdURL) +
        strlen(protocolStr) + 20 /* max int len */
        + strlen(authenticatorStr) + fUserAgentHeaderStrLen +
        strlen(extraHeaders) + strlen(contentLengthHeader) + contentStrLen;
    ...
    if (fTunnelOverHTTPPortNum != 0 &&
      ...
      // before we send it. (However, we don't do this for the HTTP "GET" and
      // "POST" commands that we use to set up the tunnel.)
      char *origCmd = cmd;
    ...
    // response (when it comes) can be handled. However, note that we do not
    // expect a response to a POST command with RTSP-over-HTTP, so don't enqueue
    // that.
...
Boolean RTSPClient::setRequestFields(RequestRecord *request, char *&cmdURL,
  ...
  if (strcmp(request->commandName(), "DESCRIBE") == 0) {
    ...
    // the server wants this to indicate client 'liveness); this makes up our
    // 'extra headers':
    ...
    sprintf(transportStr, transportFmt, transportTypeStr, modeStr, portTypeStr,
    ...
    // The "Transport:", "Session:" (if present), "Blocksize:" (if present), and
    // "KeyMgmt:" (if present) headers make up the 'extra headers':
    extraHeaders = new char[transportSize + strlen(sessionStr) +
                            strlen(blocksizeStr) + strlen(keyMgmtStr) + 1];
    extraHeadersWereAllocated = True;
    sprintf(extraHeaders, "%s%s%s%s", transportStr, sessionStr, blocksizeStr,
            keyMgmtStr);
    delete[] transportStr;
    delete[] sessionStr;
    delete[] blocksizeStr;
    ...
    // as our 'cmdURL' in the subsequent request), and the server address (which
    // we'll use in a "Host:" header):
    char *username;
    char *password;
    ...
    if (strcmp(request->commandName(), "PLAY") == 0) {
      ...
      char *rangeStr =
          createRangeString(request->start(), request->end(),
...
int RTSPClient::openConnection() {
  do {
    ...
    ignoreSigPipeOnSocket(
        fInputSocketNum); // so that servers on the same host that get killed
                          // don't also kill us
...
int RTSPClient::connectToServer(int socketNum, portNumBits remotePortNum) {
  ...
  if (fVerbosityLevel >= 1) {
    envir() << "Connecting to " << AddressString(remoteName).val() << ", port "
  ...
  // The connection succeeded.  Arrange to handle responses to requests sent on
  // it:
  envir().taskScheduler().setBackgroundHandling(
      fInputSocketNum, SOCKET_READABLE | SOCKET_EXCEPTION,
      (TaskScheduler::BackgroundHandlerProc *)&incomingDataHandler, this);
...
Boolean RTSPClient ::parseResponseCode(char const *line, unsigned &responseCode,
  ...
  // to setup RTSP-over-HTTP tunneling, and so that we get back a meaningful
  // error if the client tried to mistakenly send a RTSP command to a HTTP-only
  // server.
...
void RTSPClient::handleIncomingRequest() {
  ...
  if (!parseRTSPRequestString(
          fResponseBuffer, fResponseBytesAlreadySeen, cmdName, sizeof cmdName,
          urlPreSuffix, sizeof urlPreSuffix, urlSuffix, sizeof urlSuffix, cseq,
          sizeof cseq, sessionId, sizeof sessionId, contentLength)) {
    return;
  } else {
    if (fVerbosityLevel >= 1) {
...
Boolean RTSPClient::parseTransportParams(char const *paramsStr,
  ...
  // this as the 'server' address (because some weird servers don't specify the
...
Boolean RTSPClient::handleGET_PARAMETERResponse(char const *parameterName,
  ...
  do {
    ...
    if (parameterName != NULL && parameterName[0] != '\0') {
      ...
      if (parameterNameLen > 0 && _strncasecmp(resultValueString, parameterName,
                                               parameterNameLen) == 0) {
...
void RTSPClient::responseHandlerForHTTP_GET1(int responseCode,
  ...
  do {
    ...
    // server->client link, set up a second TCP connection (to the same server &
    // port as before) for the client->server link.  All future output will be
    // to this new socket.
    ...
    if (connectResult < 0)
      ...
      // connection completes. First, move the pending requests to the 'awaiting
      // connection' queue:
...
void RTSPClient::connectionHandler1() {
  ...
  // "fRequestsAwaitingConnection" (so that "sendRequest()" doesn't get confused
  // by "fRequestsAwaitingConnection" being nonempty, and enqueue them all over
  // again).
  ...
  do {
    ...
    if (getsockopt(fInputSocketNum, SOL_SOCKET, SO_ERROR, (char *)&err, &len) <
            0 ||
        err != 0) {
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
  do {
    ...
    // response handlers, indicating this error. (However, the "RTSP response
    // was truncated" error is applied to the first response handler only.)
    resetResponseBuffer();
    RequestRecord *request;
  ...
  do {
    ...
    // if it contains <CR><LF><CR><LF>. (If not, wait for more data to arrive.)
    ...
    // <CR><LF><CR><LF>), parse them to get the response code, CSeq, and various
    // other header parameters.  To do this, we first make a copy of the
    // received header data, because we'll be modifying it by adding '\0' bytes.
    char *headerDataCopy;
      ...
      while (1) {
        ...
        if (checkForHeader(lineStart, "CSeq:", 5, headerParamsStr)) {
          ...
          // to check whether your proposed modification is appropriate/correct,
                       ...
                       6,
                       // a future release of the "LIVE555 Streaming Media"
                       // software,
                       rangeParamsStr
                       // so that other projects that use the code could benefit
                       // (not just your own project).
                       )) {
          ...
          // it with this new one only if the new one specifies "Digest"
          // authentication:
          ...
          setBaseURL(headerParamsStr);
                                  ...
                                  headerParamsStr)) {
          // Replace the tail of the 'base URL' with the value of this header
          // parameter:
          ...
          if (oldBaseURLTail != NULL) {
            ...
            // Note: We couldn't use "asprintf()", because some compilers don't
            // support it
      ...
      if (contentLength > numBodyBytes) {
        ...
        if (numExtraBytesNeeded > remainingBufferSize) {
          ...
          sprintf(tmpBuf,
                  ...
                  "\"Content-Length:\" %d (need a buffer size of >= %d bytes\n",
      ...
      if (foundRequest != NULL) {
        ...
        if (responseCode == 200) {
          ...
          if (strcmp(foundRequest->commandName(), "GET") == 0) {
            ...
            // returns "401 Unauthorized", then we resend it (with an
            // "Authorization:" header), just as we would for a RTSP command.
            // However, we do so using a new TCP connection, because some
            // servers close the original connection after returning the "401
            // Unauthorized".
...
RTSPClient::RequestRecord::RequestRecord(
    ...
    : fNext(NULL), fCSeq(cseq), fCommandName(commandName), fSession(session),
      fSubsession(subsession), fBooleanFlags(booleanFlags), fStart(start),
      fEnd(end), fAbsStartTime(NULL), fAbsEndTime(NULL), fScale(scale),
...
RTSPClient::RequestRecord::RequestRecord(unsigned cseq,
    ...
    : fNext(NULL), fCSeq(cseq), fCommandName("PLAY"), fSession(session),
      fSubsession(subsession), fBooleanFlags(0), fStart(0.0f), fEnd(-1.0f),
      fAbsStartTime(strDup(absStartTime)), fAbsEndTime(strDup(absEndTime)),
      fScale(scale), fContentStr(NULL), fHandler(handler) {}
...
void HandlerServerForREGISTERCommand ::implementCmd_REGISTER(
  ...
  if (strcmp(cmd, "REGISTER") ==
    ...
    // Create a new "RTSPClient" object, and call our 'creation function' with
    // it:
