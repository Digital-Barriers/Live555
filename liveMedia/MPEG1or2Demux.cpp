...
MPEG1or2Demux ::MPEG1or2Demux(UsageEnvironment &env, FramedSource *inputSource,
    ...
    : Medium(env), fInputSource(inputSource), fMPEGversion(0),
...
Boolean
MPEG1or2Demux::useSavedData(u_int8_t streamIdTag, unsigned char *to,
                            unsigned maxSize,
                            FramedSource::afterGettingFunc *afterGettingFunc,
...
void MPEGProgramStreamParser::parseSystemHeader() {
  ...
  // be at least 6 bytes.  Check this now:
...
Boolean
MPEGProgramStreamParser ::isSpecialStreamId(unsigned char stream_id) const {
...
#ifdef DEBUG
      fprintf(stderr,
              "%d, %d bytes of PES_packet_data (out.maxSize: %d); first 4 "
              "bytes: 0x%08x\n",
...
#ifdef DEBUG
      fprintf(stderr,
              "%d, currently undeliverable PES data; first 4 bytes: 0x%08x - "
