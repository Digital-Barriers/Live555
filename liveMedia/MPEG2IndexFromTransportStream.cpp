...
MPEG2IFrameIndexFromTransportStream ::MPEG2IFrameIndexFromTransportStream(
    ...
    : FramedFilter(env, inputSource), fIsH264(False), fIsH265(False),
      fInputTransportPacketCounter((unsigned)-1), fClosureNumber(0),
      fLastContinuityCounter(~0), fFirstPCR(0.0), fLastPCR(0.0),
      fHaveSeenFirstPCR(False), fPMT_PID(0x10),
      fVideo_PID(0xE0), // default values
      fParseBufferSize(PARSE_BUFFER_SIZE), fParseBufferFrameStart(0),
      fParseBufferParseEnd(4), fParseBufferDataEnd(0), fHeadIndexRecord(NULL),
...
void MPEG2IFrameIndexFromTransportStream::doGetNextFrame() {
  ...
  fInputSource->getNextFrame(fInputBuffer, sizeof fInputBuffer,
                             afterGettingFrame, this, handleInputClosure, this);
}
...
void MPEG2IFrameIndexFromTransportStream ::afterGettingFrame1(
  ...
  if (totalHeaderSize > 5 && (fInputBuffer[5] & 0x10) != 0) {
    // There's a PCR:
    u_int32_t pcrBaseHigh = (fInputBuffer[6] << 24) | (fInputBuffer[7] << 16) |
...
Boolean MPEG2IFrameIndexFromTransportStream::parseFrame() {
  ...
  // representing the data in the parse buffer from "fParseBufferFrameStart" to
  // "fParseBufferDataEnd".  We now parse through this data, looking for a
  // complete 'frame', where a 'frame', in this case, means: 	for MPEG video: a
  // Video Sequence Header, GOP Header, Picture Header, or Slice 	for H.264 or
  // H.265 video: a NAL unit
  ...
  if (fIsH264) {
    ...
    default:
      curRecordType =
          (curCode <= 31) ? RECORD_NAL_H265_NON_IFRAME : RECORD_NAL_H265_OTHER;
      if (!parseToNextCode(nextCode))
        return False;
      break;
    }
...
Boolean
MPEG2IFrameIndexFromTransportStream ::parseToNextCode(unsigned char &nextCode) {
...
IndexRecord::IndexRecord(u_int8_t startOffset, u_int8_t size,
    ...
    : fNext(this), fPrev(this), fRecordType(RECORD_UNPARSED),
      fStartOffset(startOffset), fSize(size), fPCR(pcr),
