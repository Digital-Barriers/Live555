...
MIKEYState::MIKEYState()
    : // Set default encryption/authentication parameters:
      fEncryptSRTP(True), fEncryptSRTCP(True), fMKI(our_random32()),
...
MIKEYState *MIKEYState::createNew(u_int8_t *messageToParse,
...
MIKEYState::MIKEYState(u_int8_t const *messageToParse, unsigned messageSize,
                       Boolean &parsedOK)
    : // Set encryption/authentication parameters to default values (that may be
      // overwritten later as we parse the message):
  ...
  do {
    ...
    case SP: { // RFC 3830, section 6.10
      ...
      while (1) {
        ...
        case 2: { // Authentication algorithm: we handle only NULL and
                  // HMAC-SHA-1
          if (ppLength != 1)
            break;
          u_int8_t value = ptr[0];
    ...
    case KEMAC: { // RFC 3830, section 6.2
      ...
      // Parse the 'key data sub-payload':
      {
        ...
        // Note: This means that we'll reject the "a=key-mgmt" SDP MIKEY data
        // from
        //   an Axis camera, because that doesn't specify SPI/MKI.  But that's
        //   what we want, because the Axis camera's "a=key-mgmt" SDP MIKEY data
        //   is meant to be ignored by clients.
        if (*subPtr++ != ((2 << 4) | 1))
          break; // Type 2 (TEK) | KV 1 (SPI/MKI)
        u_int16_t keyDataLen = get2Bytes(subPtr);
        // The key data length must be 30 (encryption key length (16) + salt
        // length (14)):
        if (keyDataLen != 30)
          break;

        // Make sure we have enough space for the key data and the "SPI Length"
        // field:
...
MIKEYPayload::MIKEYPayload(MIKEYState &ourMIKEYState, u_int8_t payloadType)
  ...
  case SP: { // RFC 3830, section 6.10
    ...
    *p++ = 0; // Protocol type: SRTP
    ...
    add1BytePolicyParam(
        p, 2 /*Authentication algorithm*/,
        fOurMIKEYState.useAuthentication() ? 1 /*HMAC-SHA-1*/ : 0 /*NULL*/);
...
MIKEYPayload::~MIKEYPayload() { delete fNext; }
