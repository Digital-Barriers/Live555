...
struct DVVideoProfile {
  ...
  unsigned dvFrameSize; // in bytes (==
                        ...
                        // i.e. 12000))
  double frameDuration; // duration of the above, in microseconds. (1000000/this
                        // == frame rate)
};
...
void DVVideoStreamFramer::afterGettingFrame(unsigned frameSize,
  ...
  if (fOurProfile == NULL && frameSize >= DV_SAVED_INITIAL_BLOCKS_SIZE) {
    ...
    // necessarily on a (150-block) sequence boundary. We therefore scan each
    // 80-byte block, until we find the 6-block header that begins a sequence:
    ...
    for (u_int8_t const *ptr = data;
      ...
      if (sectionHeader == DV_SECTION_HEADER &&
        ...
        // DV profile:
        for (DVVideoProfile const *profile = profiles; profile->name != NULL;
             ++profile) {
