...
MatroskaFileParser::MatroskaFileParser(MatroskaFile &ourFile,
    ...
    : StreamParser(inputSource, onEndFunc, onEndClientData, continueParsing,
                   this),
      fOurFile(ourFile), fInputSource(inputSource), fOnEndFunc(onEndFunc),
      fOnEndClientData(onEndClientData), fOurDemux(ourDemux),
      ...
      fFrameSizesWithinBlock(NULL), fPresentationTimeOffset(0.0) {
  if (ourDemux == NULL) {
    // Initialization
...
void MatroskaFileParser::continueParsing() {
  if (fInputSource != NULL) {
    if (!parse()) {
      ...
      // the source, or because we're waiting for another read from downstream.
    ...
    do {
      ...
      case PARSING_TRACK: {
        ...
        if (areDone && fOurFile.fCuesOffset > 0) {
          // We've finished parsing the 'Track' information.  There are also
          // 'Cues' in the file, so parse those before finishing: Seek to the
          // specified position in the file.  We were already told that the
          // 'Cues' begins there:
#ifdef DEBUG
          fprintf(stderr,
                  "Seeking to file position %llu (the previously-reported "
                  "location of 'Cues')\n",
      ...
      case LOOKING_FOR_CLUSTER: {
        if (fOurFile.fClusterOffset > 0) {
          // Optimization: Seek to the specified position in the file.  We were
          // already told that the 'Cluster' begins there:
#ifdef DEBUG
          fprintf(stderr,
...
#ifdef DEBUG
  fprintf(stderr,
          "MatroskaFileParser::parseStartOfFile(): Parsed id 0x%s (%s), size: "
          "%lld\n",
  ...
  while (fCurrentParseState == LOOKING_FOR_TRACKS) {
    ...
    case MATROSKA_ID_SEEK_POSITION: { // 'Seek Position' header: get this value
      ...
      if (parseEBMLVal_unsigned64(size, seekPosition)) {
        ...
        fprintf(
            stderr,
            "\tSeek Position %llu (=> offset within the file: %llu (0x%llx))\n",
...
#ifdef DEBUG
        fprintf(stderr,
                "\tTimecode Scale %u ns (=> Segment Duration == %f seconds)\n",
...
Boolean MatroskaFileParser::parseTrack() {
  ...
  while (fCurOffsetInFile < fLimitOffsetInFile) {
    ...
    fprintf(
        stderr,
        "MatroskaFileParser::parseTrack(): Parsed id 0x%s (%s), size: %lld\n",
        id.hexString(), id.stringName(), size.val());
#endif
    switch (id.val()) {
        ...
        if (track != NULL) {
          ...
          if (track->codecID != NULL) {
            if (strcmp(track->codecID, "V_MPEG4/ISO/AVC") == 0) { // H.264
              ...
              // for H.264. We check for this here, by checking various fields
              // that are supposed to be 'all-1' in the 'correct' format:
    ...
    case MATROSKA_ID_CONTENT_COMPRESSION: { // 'Content Compression' header:
      ...
      // 'zlib' compression (the default algorithm). Therefore, we disable this
      // track, unless/until we later see that 'Header Stripping' is supported:
      if (track != NULL)
    ...
    case MATROSKA_ID_COLOR_SPACE: {
      ...
      if (parseEBMLVal_binary(size, colourSpace)) {
        ...
        fprintf(stderr, "\tColor space : %02x %02x %02x %02x\n", colourSpace[0],
                colourSpace[1], colourSpace[2], colourSpace[3]);
#endif
        if ((track != NULL) && (colourSpaceSize == 4)) {
          // convert to sampling value (rfc 4175)
          if ((strncmp((const char *)colourSpace, "I420", 4) == 0) ||
              (strncmp((const char *)colourSpace, "IYUV", 4) == 0)) {
            track->colorSampling = "YCbCr-4:2:0";
          } else if ((strncmp((const char *)colourSpace, "YUY2", 4) == 0) ||
                     (strncmp((const char *)colourSpace, "UYVY", 4) == 0)) {
            track->colorSampling = "YCbCr-4:2:2";
          } else if (strncmp((const char *)colourSpace, "AYUV", 4) == 0) {
            track->colorSampling = "YCbCr-4:4:4";
          } else if ((strncmp((const char *)colourSpace, "Y41P", 4) == 0) ||
                     (strncmp((const char *)colourSpace, "Y41T", 4) == 0)) {
...
void MatroskaFileParser::lookForNextBlock() {
  ...
  while (fCurrentParseState == LOOKING_FOR_BLOCK) {
    ...
    // Attachments are parsed only if we're in DEBUG mode (otherwise we just
    // skip over them):
#ifdef DEBUG
...
Boolean MatroskaFileParser::parseCues() {
  ...
  while (fCurOffsetInFile < fLimitOffsetInFile) {
    ...
    fprintf(
        stderr,
        "MatroskaFileParser::parseCues(): Parsed id 0x%s (%s), size: %lld\n",
        id.hexString(), id.stringName(), size.val());
#endif
    switch (id.val()) {
...
#ifdef DEBUG_CUES
        fprintf(stderr,
                "\tCue Cluster Position %llu (=> offset within the file: %llu "
                "(0x%llx))\n",
...
void MatroskaFileParser::parseBlock() {
  ...
  do {
    ...
    MatroskaLacingType lacingType =
        (c == 0x0) ? NoLacing
                   : (c == 0x02) ? XiphLacing
                                 : (c == 0x04) ? FixedSizeLacing : EBMLLacing;
#ifdef DEBUG
    fprintf(
        stderr, "\ttrack number %d, timecode %d (=> %f seconds), %s lacing\n",
        ...
        if (lacingType == EBMLLacing) {
          ...
          if (i == 0) {
            ...
            unsigned toSubtract =
                (fsv > 0xFFFFFF)
                    ? 0x07FFFFFF
                    : (fsv > 0xFFFF) ? 0x0FFFFF : (fsv > 0xFF) ? 0x1FFF : 0x3F;
    ...
    // bytes' that are common to (the front of) all frames, then count them now:
...
#ifdef DEBUG
    fprintf(stderr,
            "\tThis block contains %d frame(s); size(s):", fNumFramesInBlock);
    unsigned frameSizesTotal = 0;
    for (unsigned i = 0; i < fNumFramesInBlock; ++i) {
...
Boolean MatroskaFileParser::deliverFrameWithinBlock() {
  ...
  do {
    ...
    if (track->codecIsOpus && demuxedTrack->fOpusTrackNumber < 2) {
      // Special case for Opus audio.  The first frame (the 'configuration'
      // header) comes from the 'private data'.  The second frame (the 'comment'
      // header) comes is synthesized by us here:
    ...
    if (fPresentationTimeOffset == 0.0) {
      ...
      // offset to make the presentation times aligned with 'wall clock' time:
    ...
    if (specialFrameSource != NULL) {
      ...
      if (track->haveSubframes()) {
        ...
        if (fCurOffsetWithinFrame + frameSize + track->subframeSizeSize <
            fFrameSizesWithinBlock[fNextFrameNumberToDeliver]) {
          // There's room for at least one more subframe after this, so give
          // this subframe a duration of 0
          durationInMicroseconds = 0;
        }
      }
    }
    ...
    if (track->defaultDuration == 0) {
      ...
      if (demuxedTrack->durationImbalance() > 0) {
        ...
        int const adjustmentThreshold =
            100000; // don't increase the duration by more than this amount (in
  ...
  fprintf(
      stderr,
      "deliverFrameWithinBlock(): Error parsing data; trying to recover...\n");
#endif
  fCurrentParseState = LOOKING_FOR_BLOCK;
  return True;
}
  ...
  if (track->headerStrippedBytesSize > fCurOffsetWithinFrame) {
    ...
    if (numBytesToGet <= numRemainingHeaderStrippedBytes) {
      ...
      if (numBytesToGet + numBytesToSkip <= numRemainingHeaderStrippedBytes) {
        ...
        numBytesToSkip =
            numBytesToGet + numBytesToSkip - numRemainingHeaderStrippedBytes;
      }
    } else {
...
void MatroskaFileParser::skipRemainingHeaderBytes(Boolean isContinuation) {
  ...
  // excessively large distance, break up the skipping into manageable chunks,
