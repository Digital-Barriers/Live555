...
OggFileSink::OggFileSink(UsageEnvironment &env, FILE *fid,
    ...
    : FileSink(env, fid, bufferSize, perFrameFileNamePrefix),
      ...
      fHaveWrittenFirstFrame(False), fHaveSeenEOF(False), fGranulePosition(0),
      fGranulePositionAdjustment(0), fPageSequenceNumber(0), fIsTheora(False),
      fGranuleIncrementPerFrame(1), fAltFrameSize(0), fAltNumTruncatedBytes(0) {
...
Boolean OggFileSink::continuePlaying() {
  ...
  fSource->getNextFrame(fBuffer, fBufferSize, FileSink::afterGettingFrame, this,
...
void OggFileSink::addData(unsigned char const *data, unsigned dataSize,
  ...
  // pages if it's too big for a single page).  We don't aggregate more than one
  // frame within an Ogg page because that's not legal for some headers, and
  // because that would make it difficult for us to properly set the 'eos' (end
  // of stream) flag on the last page.
  ...
  // will be an extra 0-size page at the end
  for (unsigned i = 0; i < numPagesToWrite; ++i) {
    ...
    if (i < numPagesToWrite - 1) {
      ...
      fPageHeaderBytes[6] = fPageHeaderBytes[7] = fPageHeaderBytes[8] =
          fPageHeaderBytes[9] = fPageHeaderBytes[10] = fPageHeaderBytes[11] =
    ...
    if (dataSize >= PAGE_DATA_MAX_SIZE) {
      ...
      number_page_segments = (pageDataSize + 255) /
  ...
  if (!fHaveWrittenFirstFrame) {
    ...
    if (fConfigStr != NULL && fConfigStr[0] != '\0') {
      ...
      parseVorbisOrTheoraConfigStr(
          fConfigStr, identificationHdr, identificationHdrSize, commentHdr,
