...
Boolean RTSPRegisterSender::setRequestFields(
  ...
  if (strcmp(request->commandName(), "REGISTER") == 0) {
    ...
    // parameters.  This will be "extraHeaders". First, generate the
    // "proxy_url_suffix" parameter string, if any:
...
RTSPDeregisterSender *RTSPDeregisterSender ::createNew(
  ...
  return new RTSPDeregisterSender(
      env, remoteClientNameOrAddress, remoteClientPortNum, rtspURLToDeregister,
      rtspResponseHandler, authenticator, proxyURLSuffix, verbosityLevel,
      applicationName);
}
