...
void updateSideInfoForHuffman(
  ...
  if (isMPEG2) {
    ...
    MP3HuffmanDecode(gr, isMPEG2, mainDataPtr, granule1Offset, origTotABsize,
                     sfLength, hei);
...
static int read_decoder_table(unsigned char *fi) {
  ...
  for (n = 0; n < HTN; n++) {
    ...
    if (strcmp(command, ".reference") == 0) {
      ...
      rsf_ht[n].ref = -1;
      rsf_ht[n].val =
          (unsigned char(*)[2]) new unsigned char[2 * (rsf_ht[n].treelen)];
...
static int
rsf_huffman_decoder(BitVector &bv,
