...
ServerMediaSession *
ServerMediaSession ::createNew(UsageEnvironment &env, char const *streamName,
...
ServerMediaSession::ServerMediaSession(UsageEnvironment &env,
    ...
    : Medium(env), fIsSSM(isSSM), fSubsessionsHead(NULL),
      fSubsessionsTail(NULL), fSubsessionCounter(0), fReferenceCount(0),
...
void ServerMediaSession::testScaleFactor(float &scale) {
  ...
  // value.  If the subsessions' actual scales still differ, re-set them all
  // to 1.
...
float ServerMediaSession::duration() const {
  ...
  for (ServerMediaSubsession *subsession = fSubsessionsHead; subsession != NULL;
    ...
    // a negative value, to indicate that only subsessions will have a
    // "a=range:" attribute:
