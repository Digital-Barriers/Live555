...
H264or5VideoStreamFramer ::H264or5VideoStreamFramer(
    ...
    : MPEGVideoStreamFramer(env, inputSource), fHNumber(hNumber),
      fIncludeStartCodeInOutput(includeStartCodeInOutput),
      fInsertAccessUnitDelimiters(insertAccessUnitDelimiters),
      fLastSeenVPS(NULL), fLastSeenVPSSize(0), fLastSeenSPS(NULL),
      fLastSeenSPSSize(0), fLastSeenPPS(NULL), fLastSeenPPSSize(0) {
...
#define SPS_MAX_SIZE                                                           \
  1000 // larger than the largest possible SPS (Sequence Parameter Set) NAL unit
...
void H264or5VideoStreamFramer::doGetNextFrame() {
  if (fInsertAccessUnitDelimiters && pictureEndMarker()) {
    ...
    if (fHNumber == 264) {
      ...
      *fTo++ = 0; // "nuh_layer_id" (0); "nuh_temporal_id_plus1" (0) (Is this
                  // correct??)
      *fTo++ =
          0x50; // "pic_type" (2); "rbsp_trailing_bits()" (Is this correct??)
    }
...
H264or5VideoStreamParser ::H264or5VideoStreamParser(
    ...
    : MPEGVideoStreamParser(usingSource, inputSource), fHNumber(hNumber),
      ...
      fParsedFrameRate(0.0), cpb_removal_delay_length_minus1(23),
      dpb_output_delay_length_minus1(23), CpbDpbDelaysPresentFlag(0),
      pic_struct_present_flag(0), DeltaTfiDivisor(2.0) {}
...
Boolean
H264or5VideoStreamParser::usuallyBeginsAccessUnit(u_int8_t nal_unit_type) {
...
void H264or5VideoStreamParser ::analyze_video_parameter_set_data(
  ...
  // Assert: fHNumber == 265 (because this function is called only when parsing
  // H.265)
  unsigned i;
...
void H264or5VideoStreamParser ::analyze_sei_payload(unsigned payloadType,
  ...
  if (payloadType == 1 /* pic_timing, for both H.264 and H.265 */) {
    ...
    if (pic_struct_present_flag) {
      ...
      // Use this to set "DeltaTfiDivisor" (which is used to compute the frame
      // rate):
      if (fHNumber == 264) {
        DeltaTfiDivisor =
            pic_struct == 0
    ...
    // If "DeltaTfiDivisor" has changed, and we've already computed the frame
    // rate, then adjust it, based on the new value of "DeltaTfiDivisor":
...
unsigned H264or5VideoStreamParser::parse() {
  try {
    ...
    // Then save everything up until the next 0x00000001 (4 bytes) or 0x000001
    // (3 bytes), or we hit EOF. Also make note of the first byte, because it
    // contains the "nal_unit_type":
    if (haveSeenEOF()) {
      // We hit EOF the last time that we tried to parse this data, so we know
      // that any remaining unparsed data forms a complete NAL unit, and that
      // there's no 'start code' at the end:
      ...
      (void)get1Byte(); // forces another read, which will cause EOF to get
                        // handled for real this time
      return 0;
    } else {
      ...
      // all previous bytes (forming a complete NAL unit). Skip over these
      // remaining bytes, up until the start of the next NAL unit:
    ...
    if (fHNumber == 264) {
      ...
      u_int8_t nal_ref_idc = (fFirstByteOfNALUnit & 0x60) >> 5;
      fprintf(stderr,
              "Parsed %d-byte NAL-unit (nal_ref_idc: %d, nal_unit_type: %d "
              "(\"%s\"))\n",
    ...
    if (isVPS(nal_unit_type)) { // Video parameter set
      // First, save a copy of this NAL unit, in case the downstream object
      // wants to see it:
      ...
      if (fParsedFrameRate == 0.0) {
        ...
        // So parse this NAL unit to check whether frame rate information is
        // present:
        ...
        if (time_scale > 0 && num_units_in_tick > 0) {
          ...
          fprintf(
              stderr,
              "\tThis \"Video Parameter Set\" NAL unit contained no frame rate "
              "information, so we use a default frame rate of %f fps\n",
              usingSource()->fFrameRate);
#endif
        }
      }
      ...
      if (fParsedFrameRate == 0.0) {
        ...
        if (time_scale > 0 && num_units_in_tick > 0) {
          ...
          fprintf(
              ...
              "rate information, so we use a default frame rate of %f fps\n",
              usingSource()->fFrameRate);
#endif
        }
      }
    } else if (isPPS(nal_unit_type)) { // Picture parameter set
      // Save a copy of this NAL unit, in case the downstream object wants to
      // see it:
      usingSource()->saveCopyOfPPS(fStartOfFrame + fOutputStartCodeSize,
                                   curFrameSize() - fOutputStartCodeSize);
    } else if (isSEI(nal_unit_type)) { // Supplemental enhancement information
                                       // (SEI)
      analyze_sei_data(nal_unit_type);
      // Later, perhaps adjust "fPresentationTime" if we saw a "pic_timing" SEI
      // payload??? #####
    }
    ...
    // (RTP streamers need to know this in order to figure out whether or not to
    // set the "M" bit.)
    Boolean thisNALUnitEndsAccessUnit;
    if (haveSeenEOF() || isEOF(nal_unit_type)) {
      // There is no next NAL unit, so we assume that this one ends the current
      // 'access unit':
      thisNALUnitEndsAccessUnit = True;
    } else if (usuallyBeginsAccessUnit(nal_unit_type)) {
      // These NAL units usually *begin* an access unit, so assume that they
      // don't end one here:
      ...
      if (isVCL(next_nal_unit_type)) {
        ...
        // The next NAL unit's type is one that usually appears at the start of
        // an 'access unit', so we assume that the current NAL unit ends an
        // 'access unit':
        thisNALUnitEndsAccessUnit = True;
      } else {
...
#ifdef DEBUG
    fprintf(stderr, "H264or5VideoStreamParser::parse() EXCEPTION (This is "
