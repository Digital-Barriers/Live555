...
public:
  AVIIndexRecord(unsigned chunkId, unsigned flags, unsigned offset,
                 unsigned size)
      : fNext(NULL), fChunkId(chunkId), fFlags(flags), fOffset(offset),
        fSize(size) {}
...
AVIFileSink::AVIFileSink(UsageEnvironment &env, MediaSession &inputSession,
    ...
    : Medium(env), fInputSession(inputSession), fIndexRecordsHead(NULL),
      fIndexRecordsTail(NULL), fNumIndexRecords(0), fBufferSize(bufferSize),
      fPacketLossCompensate(packetLossCompensate),
      fAreCurrentlyBeingPlayed(False), fNumSubsessions(0), fNumBytesWritten(0),
      fHaveCompletedOutputFile(False), fMovieWidth(movieWidth),
...
AVIFileSink *
AVIFileSink ::createNew(UsageEnvironment &env, MediaSession &inputSession,
                        char const *outputFileName, unsigned bufferSize,
                        unsigned short movieWidth, unsigned short movieHeight,
                        unsigned movieFPS, Boolean packetLossCompensate) {
  AVIFileSink *newSink =
      new AVIFileSink(env, inputSession, outputFileName, bufferSize, movieWidth,
...
void AVIFileSink ::afterGettingFrame(void *clientData, unsigned packetDataSize,
  ...
  if (numTruncatedBytes > 0) {
    ioState->envir()
        << "AVIFileSink::afterGettingFrame(): The input frame data was too "
           "large for our buffer.  "
        << numTruncatedBytes
...
AVISubsessionIOState::AVISubsessionIOState(AVIFileSink &sink,
                                           MediaSubsession &subsession)
    : fOurSink(sink), fOurSubsession(subsession), fMaxBytesPerSecond(0),
      fIsVideo(False), fIsAudio(False), fIsByteSwappedAudio(False),
      fNumFrames(0) {
...
void AVISubsessionIOState::useFrame(SubsessionBuffer &buffer) {
  ...
  if (strcmp(fOurSubsession.codecName(), "H264") == 0) {
    ...
    fOurSink.fNumBytesWritten +=
        fOurSink.addWord(fourChar(0x00, 0x00, 0x00, 0x01)); // add start code
  } else {
...
addFileHeader1(strh);
size += add4ByteString(fCurrentIOState->fIsVideo
                           ? "vids"
