...
PassiveServerMediaSubsession ::PassiveServerMediaSubsession(
    ...
    : ServerMediaSubsession(rtpSink.envir()), fSDPLines(NULL),
      fRTPSink(rtpSink), fRTCPInstance(rtcpInstance) {
  ...
  if (fRTCPInstance != NULL) {
    ...
    if (source != NULL) {
      fRTCPInstance->setSpecificRRHandler(
          source->addr, source->port, rtcpRRHandler, rtcpRRHandlerClientData);
    }
  }
}
