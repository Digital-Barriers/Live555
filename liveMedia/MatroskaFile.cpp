...
public:
  ...
  static void fprintf(FILE *fid,
                      CuePoint *cuePoint); // used for debugging; it's static to
                                           // allow for "cuePoint == NULL"
...
MatroskaFile::MatroskaFile(UsageEnvironment &env, char const *fileName,
    ...
    : Medium(env), fFileName(strDup(fileName)), fOnCreation(onCreation),
      ...
      fPreferredLanguage(strDup(preferredLanguage)), fTimecodeScale(1000000),
      fSegmentDuration(0.0), fSegmentDataOffset(0), fClusterOffset(0),
      fCuesOffset(0), fCuePoints(NULL), fChosenVideoTrackNumber(0),
      fChosenAudioTrackNumber(0), fChosenSubtitleTrackNumber(0) {
...
void MatroskaFile::getH264ConfigData(MatroskaTrack const *track, u_int8_t *&sps,
  ...
  do {
    ...
    // Use our track's 'Codec Private' data: Bytes 5 and beyond contain SPS and
    // PPSs:
...
void MatroskaFile::getH265ConfigData(MatroskaTrack const *track, u_int8_t *&vps,
  ...
  do {
    ...
    // Extract, from "VPS_SPS_PPSBytes", one VPS NAL unit, one SPS NAL unit, and
    // one PPS NAL unit. (I hope one is all we need of each.)
    ...
    if (track->codecPrivateUsesH264FormatForH265) {
      ...
      for (unsigned j = 0; j < numOfArrays; ++j) {
        ++ptr;
        CHECK_PTR; // skip the 'array_completeness'|'reserved'|'NAL_unit_type'
                   // byte
...
void MatroskaFile ::getVorbisOrTheoraConfigData(
  ...
  do {
    ...
    // configuration information, containing the "Identification", "Comment",
    // and "Setup" headers. Extract these headers now:
    Boolean isTheora =
...
RTPSink *MatroskaFile ::createRTPSinkForTrackNumber(
  ...
  do {
    ...
    if (strcmp(track->mimeType, "audio/L16") == 0) {
      ...
      result = SimpleRTPSink::createNew(
          envir(), rtpGroupsock, rtpPayloadTypeIfDynamic,
          track->samplingFrequency, "audio", "PCMU", track->numChannels);
      ...
      // configuration information.  Use this to generate a hexadecimal 'config'
      // string for the new RTP sink:
      ...
      getVorbisOrTheoraConfigData(
          track, identificationHeader, identificationHeaderSize, commentHeader,
          commentHeaderSize, setupHeader, setupHeaderSize);
      ...
      result = RawVideoRTPSink::createNew(
          envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, track->pixelHeight,
          track->pixelWidth, track->bitDepth, track->colorSampling,
          track->colorimetry);
    } else if (strcmp(track->mimeType, "video/H264") == 0) {
      u_int8_t *sps;
      unsigned spsSize;
      ...
      result = H265VideoRTPSink::createNew(envir(), rtpGroupsock,
                                           rtpPayloadTypeIfDynamic, vps,
...
FileSink *MatroskaFile::createFileSinkForTrackNumber(unsigned trackNumber,
  ...
  do {
    ...
    if (createOggFileSink) {
      ...
      result =
          OggFileSink::createNew(envir(), fileName, track->samplingFrequency,
                                 configStr, MAX_KEY_FRAME_SIZE);
      delete[] configStr;
    } else if (result == NULL) {
...
MatroskaTrack::MatroskaTrack()
    : trackNumber(0 /*not set*/), trackType(0 /*unknown*/), isEnabled(True),
      isDefault(True), isForced(False), defaultDuration(0), name(NULL),
      language(NULL), codecID(NULL), samplingFrequency(0), numChannels(2),
      mimeType(""), codecPrivateSize(0), codecPrivate(NULL),
      codecPrivateUsesH264FormatForH265(False), codecIsOpus(False),
      headerStrippedBytesSize(0), headerStrippedBytes(NULL), colorSampling(""),
...
FramedSource *
MatroskaDemux::newDemuxedTrackByTrackNumber(unsigned trackNumber) {
  if (trackNumber == 0)
    return NULL;
...
CuePoint::CuePoint(double cueTime, u_int64_t clusterOffsetInFile,
                   unsigned blockNumWithinCluster)
    : fBalance(0), fCueTime(cueTime), fClusterOffsetInFile(clusterOffsetInFile),
...
Boolean CuePoint::lookup(double &cueTime, u_int64_t &resultClusterOffsetInFile,
  ...
  if (cueTime < fCueTime) {
    ...
    if (right() == NULL || !right()->lookup(cueTime, resultClusterOffsetInFile,
                                            resultBlockNumWithinCluster)) {
      // Use this record:
      cueTime = fCueTime;
