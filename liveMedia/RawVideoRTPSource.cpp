...
Boolean
RawVideoRTPSource ::processSpecialHeader(BufferedPacket *packet,
  ...
  while (1) {
    ...
    // Check the "C" (continuation) bit of this header to see whether any more
    // line headers follow:
...
void RawVideoBufferedPacket::getNextEnclosedFrameParameters(
  ...
  if (fOurSource->fNextLine >= fOurSource->fNumLines) {
    fOurSource->envir()
        << "RawVideoBufferedPacket::nextEnclosedFrameParameters(" << dataSize
        << "): data error (" << fOurSource->fNextLine
