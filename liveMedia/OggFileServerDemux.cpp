...
OggFileServerDemux ::newServerMediaSubsession(u_int32_t &resultTrackNumber) {
  resultTrackNumber = 0;
  ...
  if (clientSessionId != 0 && clientSessionId == fLastClientSessionId) {
    demuxToUse =
        fLastCreatedDemux; // use the same demultiplexor as before
                           ...
                           // creation of streams for different client sessions
                           // do not overlap - so all demuxed tracks are created
                           // for one "OggDemux" at a time. Also, the
                           // "clientSessionId != 0" test is a hack, because
                           // 'session 0' is special; its audio and video
                           // streams are created and destroyed one-at-a-time,
                           // rather than both streams being created, and then
                           // (later) both streams being destroyed (as is the
                           // case for other ('real') session ids).  Because of
                           // this, a separate demultiplexor is used for each
                           // 'session 0' track.
  }
...
OggFileServerDemux ::OggFileServerDemux(UsageEnvironment &env,
                                        ...
                                        onCreationFunc *onCreation,
                                        void *onCreationClientData)
    : Medium(env), fFileName(fileName), fOnCreation(onCreation),
      fOnCreationClientData(onCreationClientData),
      fIter(NULL /*until the OggFile is created*/), fLastClientSessionId(0),
