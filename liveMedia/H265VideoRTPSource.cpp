...
H265VideoRTPSource ::H265VideoRTPSource(UsageEnvironment &env, Groupsock *RTPgs,
    ...
    : MultiFramedRTPSource(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency,
      ...
      fExpectDONFields(expectDONFields), fPreviousNALUnitDON(0),
      fCurrentNALUnitAbsDon((u_int64_t)(~0)) {}

H265VideoRTPSource::~H265VideoRTPSource() {}

Boolean
H265VideoRTPSource ::processSpecialHeader(BufferedPacket *packet,
  ...
  case 49: { // Fragmentation Unit (FU)
    ...
    // and (optionally) the 2-byte DONL header. If the start bit is set, we
    // reconstruct the original NAL header at the end of these 3 (or 5) bytes,
    // and skip over the first 1 (or 3) bytes.
    if (packetSize < 3)
      return False;
...
BufferedPacket *
H265BufferedPacketFactory ::createNewPacket(MultiFramedRTPSource *ourSource) {
