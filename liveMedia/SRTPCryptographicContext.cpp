...
Boolean SRTPCryptographicContext ::processIncomingSRTPPacket(
  ...
  do {
    if (inPacketSize <
        12) { // For SRTP, 12 is the minimum packet size (if unauthenticated)
#ifdef DEBUG
      fprintf(stderr,
              "SRTPCryptographicContext::processIncomingSRTPPacket(): Error: "
              "Packet size %d is too short for SRTP!\n",
              inPacketSize);
#endif
      break;
    }
    ...
    if (inPacketSize <= numBytesPastEncryption) {
      ...
      fprintf(
          stderr,
          "SRTPCryptographicContext::processIncomingSRTPPacket(): Error: "
          "Packet size %d is too short for encrpytion and/or authentication!\n",
          inPacketSize);
#endif
      break;
    }
    ...
    if (weAuthenticate()) {
      ...
      if (!verifySRTPAuthenticationTag(buffer, numBytesToAuthenticate,
                                       thisPacketsROC, authenticationTag)) {
#ifdef DEBUG
        fprintf(stderr, "SRTPCryptographicContext::processIncomingSRTPPacket():"
    ...
    if (weEncryptSRTP()) {
      ...
      if ((buffer[0] & 0x10) != 0) {
        ...
        if (inPacketSize < rtpHeaderSize + 4) {
          ...
          fprintf(stderr,
                  ...
                  "Error: Packet size %d is shorter than the minimum specified "
                  "RTP header size %d!\n",
      ...
      if (offsetToEncryptedBytes > numEncryptedBytes) {
        ...
        fprintf(stderr,
                "SRTPCryptographicContext::processIncomingSRTPPacket(): Error: "
                "RTP header size %d (expected <= %d) is too large!\n",
  ...
  do {
...
#ifdef DEBUG
      fprintf(stderr,
              "SRTPCryptographicContext::processIncomingSRTCPPacket(): Error: "
              "Packet size %d is too short for SRTCP!\n",
              inPacketSize);
#endif
      break;
    }
    ...
    if (weAuthenticate()) {
      ...
      if (!verifySRTCPAuthenticationTag(buffer, numBytesToAuthenticate,
                                        authenticationTag)) {
#ifdef DEBUG
        fprintf(stderr, "SRTPCryptographicContext::processIncomingSRTCPPacket()"
...
Boolean SRTPCryptographicContext ::processOutgoingSRTCPPacket(
  ...
  do {
    ...
    // Generate and add an authentication tag over the data built so far (except
    // the MKI)
...
void SRTPCryptographicContext::cryptData(derivedKeys &keys, u_int64_t index,
  ...
  // encrypting the IV using our cipher key.  (After each step, we increment the
  // IV by 1.) We then XOR the keystream into the provided data, to do the
  // en/decryption.
  AES_KEY key;
  ...
  while (numDataBytes > 0) {
    ...
    unsigned numBytesToUse = numDataBytes < SRTP_CIPHER_KEY_LENGTH
                                 ? numDataBytes
...
void SRTPCryptographicContext ::deriveSingleKey(u_int8_t const *masterKey,
                                                ...
                                                u_int8_t *resultKey) {
  // This looks a little different from the mechanism described in RFC 3711,
  // section 4.3, but it's what the 'libsrtp' code does, so I hope it's
  // functionally equivalent:
  AES_KEY key;
  AES_set_encrypt_key(masterKey, 8 * SRTP_CIPHER_KEY_LENGTH, &key);

  u_int8_t counter[KDF_PRF_CIPHER_BLOCK_LENGTH];
  // Set the first bytes of "counter" to be the 'salt'; set the remainder to
  // zero:
  ...
  while (numBytesRemaining > 0) {
    ...
    unsigned numBytesToCopy = numBytesRemaining < KDF_PRF_CIPHER_BLOCK_LENGTH
                                  ? numBytesRemaining
                                  : KDF_PRF_CIPHER_BLOCK_LENGTH;
