...
#endif
            NULL),
        fOurServer(ourServer), fRequestId(requestId),
...
public:
  ...
  void handleResponse(int resultCode, char *resultString) {
    ...
    // We're completely done with the DEREGISTER command now, so delete ourself
    // now:
    Medium::close(this);
  }
...
RTSPServer::RTSPClientConnection::ParamsForREGISTER ::ParamsForREGISTER(
    ...
    : fCmd(strDup(cmd)), fOurConnection(ourConnection), fURL(strDup(url)),
      fURLSuffix(strDup(urlSuffix)), fReuseConnection(reuseConnection),
      fDeliverViaTCP(deliverViaTCP), fProxyURLSuffix(strDup(proxyURLSuffix)) {}
...
void RTSPServer ::RTSPClientConnection::handleCmd_REGISTER(
  ...
  if (fOurRTSPServer.weImplementREGISTER(cmd, proxyURLSuffix, responseStr)) {
    ...
    // We implement the "REGISTER"/"DEREGISTER" command by first replying to it,
    // then actually handling it (in a separate event-loop task, that will get
    // called after the reply has been done). Hack: If we're going to reuse the
    // command's connection for subsequent RTSP commands, then we delay the
    // actual handling of the command slightly, to make it less likely that the
    // first subsequent RTSP command (e.g., "DESCRIBE") will end up in the
    // client's reponse buffer before the socket (at the far end) gets reused
    // for RTSP command handling.
  ...
  while (sscanf(fields, "%[^;\r\n]", field) == 1) {
    if (strcmp(field, "reuse_connection") == 0) {
      reuseConnection = True;
               ...
               0) {
      deliverViaTCP = False;
    } else if (_strncasecmp(field, "preferred_delivery_protocol=interleaved",
                            39) == 0) {
      deliverViaTCP = True;
...
void RTSPServer::RTSPClientConnection::continueHandlingREGISTER1(
  ...
  RTSPServer *ourServer =
      &fOurRTSPServer; // copy the pointer now, in case we "delete this" below

  if (socketNumToBackEndServer >= 0) {
    // Because our socket will no longer be used by the server to handle
    // incoming requests, we can now delete this "RTSPClientConnection" object.
    // We do this now, in case the "implementCmd_REGISTER()" call below would
    // also end up deleting this.
  ...
  ourServer->implementCmd_REGISTER(
      params->fCmd, params->fURL, params->fURLSuffix, socketNumToBackEndServer,
...
void RTSPServerWithREGISTERProxying ::implementCmd_REGISTER(
  ...
  if (strcmp(cmd, "REGISTER") == 0) {
    ...
    ServerMediaSession *sms = ProxyServerMediaSession::createNew(
        envir(), this, url, proxyStreamName, fBackEndUsername, fBackEndPassword,
