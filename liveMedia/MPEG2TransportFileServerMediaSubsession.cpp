...
void MPEG2TransportFileServerMediaSubsession ::seekStream(
  ...
  OnDemandServerMediaSubsession::seekStream(clientSessionId, streamToken,
                                            seekNPT, streamDuration, numBytes);
...
ClientTrickPlayState::ClientTrickPlayState(
    MPEG2TransportStreamIndexFile *indexFile)
    : fIndexFile(indexFile), fOriginalTransportStreamSource(NULL),
      fTrickModeFilter(NULL), fTrickPlaySource(NULL), fFramer(NULL),
      fScale(1.0f), fNextScale(1.0f), fNPT(0.0f), fTSRecordNum(0),
      fIxRecordNum(0) {}
  ...
  if (streamDuration > 0.0) {
    ...
    if (streamDuration > 0.0) {
      ...
      if (fNextScale == 1.0f) {
        ...
        // stream, so instead set a PCR limit in the trick play stream.  (We
        // rely upon the fact that PCRs in the trick play stream start at 0.0)
