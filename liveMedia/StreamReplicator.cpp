...
protected:
  ...
  StreamReplica(StreamReplicator &
...
StreamReplicator *
StreamReplicator::createNew(UsageEnvironment &env, FramedSource *inputSource,
...
StreamReplicator::StreamReplicator(UsageEnvironment &env,
    ...
    : Medium(env), fInputSource(inputSource),
      ...
      fInputSourceHasClosed(False), fNumReplicas(0), fNumActiveReplicas(0),
      fNumDeliveriesMadeSoFar(0), fFrameIndex(0), fMasterReplica(NULL),
      fReplicasAwaitingCurrentFrame(NULL), fReplicasAwaitingNextFrame(NULL) {}
...
void StreamReplicator::getNextFrame(StreamReplica *replica) {
  ...
  if (fMasterReplica == NULL) {
    ...
    // 'master' replica - meaning that we read the frame into its buffer, and
    // then copy from this into the other replicas' buffers.
...
void StreamReplicator::deactivateStreamReplica(
  ...
  if (replicaBeingDeactivated->fFrameIndex == -1)
    return; // this replica has already been deactivated (or was never activated
            // at all)
  ...
  if (replicaBeingDeactivated == fMasterReplica) {
      ...
      if (fInputSource->isCurrentlyAwaitingData()) {
        ...
        if (fMasterReplica != NULL) {
          fInputSource->getNextFrame(
              fMasterReplica->fTo, fMasterReplica->fMaxSize, afterGettingFrame,
        ...
        if (fMasterReplica != NULL) {
          ...
          // frame to any new replica that might ask for it. Fortunately this
    ...
    // Check for the possibility that - now that a replica has been deactivated
    // - all other replicas have received the current frame, and so now we need
    // to complete delivery to the master replica:
...
void StreamReplicator::deliverReceivedFrame() {
  ...
  // this frame. Then, if no more requests for this frame are expected, complete
  // delivery to the 'master replica' itself.
  StreamReplica *replica;
  while ((replica = fReplicasAwaitingCurrentFrame) != NULL) {
    fReplicasAwaitingCurrentFrame = replica->fNext;
    replica->fNext = NULL;
    ...
    replica->fFrameIndex =
        1 - replica->fFrameIndex; // toggle it (0<->1), because this replica no
  ...
  if (fNumDeliveriesMadeSoFar == fNumActiveReplicas - 1 &&
    ...
    if (fReplicasAwaitingNextFrame != NULL) {
      ...
      // Arrange to read the next frame into this replica's buffer:
      if (fInputSource != NULL)
        fInputSource->getNextFrame(fMasterReplica->fTo,
                                   fMasterReplica->fMaxSize, afterGettingFrame,
                                   this, onSourceClosure, this);
    }

    // Move any other replicas that had already requested the next frame to the
    // 'requesting current frame' list: Assert: fReplicasAwaitingCurrentFrame ==
    // NULL;
    if (!(fReplicasAwaitingCurrentFrame == NULL))
      fprintf(stderr, "StreamReplicator::deliverReceivedFrame() Internal Error "
                      "3!\n"); // should not happen
...
StreamReplica::StreamReplica(StreamReplicator &ourReplicator)
    : FramedSource(ourReplicator.envir()), fOurReplicator(ourReplicator),
