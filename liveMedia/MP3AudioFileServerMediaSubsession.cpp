...
MP3AudioFileServerMediaSubsession ::MP3AudioFileServerMediaSubsession(
    ...
    : FileServerMediaSubsession(env, fileName, reuseFirstSource),
      fGenerateADUs(generateADUs), fInterleaving(interleaving),
      fFileDuration(0.0) {}
  ...
  do {
    ...
    if (fGenerateADUs) {
      ...
      if (fInterleaving != NULL) {
        ...
        streamSource =
            MP3ADUinterleaver::createNew(envir(), *fInterleaving, streamSource);
        if (streamSource == NULL)
          break;
      }
      ...
      // without tripping over the MP3 'bit reservoir':
      streamSource = ADUFromMP3Source::createNew(envir(), streamSource);
      if (streamSource == NULL)
        break;
