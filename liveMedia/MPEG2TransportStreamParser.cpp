...
Boolean MPEG2TransportStreamParser::parse() {
  ...
  try {
    while (1) {
      ...
      // Parse and process each (remaining 187 bytes of a) 'Transport Stream
      // Packet' at a time. (Because these are a lot smaller than the
      // "StreamParser" BANK_SIZE, we don't save
      //  parser state in the middle of processing each such 'Transport Stream
      //  Packet'. Therefore, processing of each 'Transport Stream Packet' needs
      //  to be idempotent.)

      u_int16_t flagsPlusPID = get2Bytes();
      // Check the "transport_error_indicator" flag; reject the packet if it's
      // set:
      if ((flagsPlusPID & 0x8000) != 0) {
#ifdef DEBUG_ERRORS
        fprintf(stderr, "MPEG2TransportStreamParser::parse() Rejected packet "
      ...
      // Reject any packets where the "transport_scrambling_control" field is
      // not zero:
      if ((controlPlusContinuity_counter & 0xC0) != 0) {
#ifdef DEBUG_ERRORS
        fprintf(stderr,
                "MPEG2TransportStreamParser::parse() Rejected packet with "
                "\"transport_scrambling_control\" set to non-zero value %d!\n",
      ...
      if (numDataBytes > 0) {
        ...
        if (!processDataBytes(PID, pusi, numDataBytes)) {
          // The parsing got deferred (to be resumed later when a pending read
          // happens)
          restoreSavedParserState(); // so that we later resume parsing at the
                                     // start of the packet
          return False;
        }
      }
...
#ifdef DEBUG_CONTENTS
    fprintf(stderr, "MPEG2TransportStreamParser::parse() EXCEPTION (This is "
...
u_int8_t MPEG2TransportStreamParser::parseAdaptationField() {
  ...
  if (adaptation_field_length > 0) {
    ...
    if ((flags & 0x10) != 0) { // PCR_flag
      ...
      // "last16PCRBits" (33 bits) program_clock_reference_extension = last 9
      // bits of "last16PCRBits" (9 bits)
    ...
    if ((flags & 0x08) != 0) { // OPCR_flag
      ...
      // original_program_clock_reference_base = "first32OPCRBits" and high bit
      // of "last16OPCRBits" (33 bits)
...
Boolean MPEG2TransportStreamParser ::processDataBytes(u_int16_t PID,
  ...
  default
      : { // Never reached, but eliminates a possible error with dumb compilers
    return False;
  }
  }
}
