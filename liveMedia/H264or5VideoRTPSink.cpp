...
void H264or5VideoRTPSink::doSpecialFrameHandling(
  ...
  // fragment of) an NAL unit, and 2/ This NAL unit was the last NAL unit of an
  // 'access unit' (i.e. video frame).
  if (fOurFragmenter != NULL) {
...
H264or5Fragmenter::H264or5Fragmenter(int hNumber, UsageEnvironment &env,
                                     ...
                                     unsigned maxOutputPacketSize)
    : FramedFilter(env, inputSource), fHNumber(hNumber),
...
void H264or5Fragmenter::doGetNextFrame() {
  if (fNumValidDataBytes == 1) {
    ...
    //    data, as a FU packet, with one extra preceding header byte (for the
    //    "FU header").
    ...
    if (fCurDataOffset == 1) {                  // case 1 or 2
      if (fNumValidDataBytes - 1 <= fMaxSize) { // case 1
        ...
        // the front of the packet (overwriting the existing "NAL header").
        if (fHNumber == 264) {
          fInputBuffer[0] = (fInputBuffer[1] & 0xE0) | 28; // FU indicator
          fInputBuffer[1] =
              0x80 | (fInputBuffer[1] & 0x1F); // FU header (with S bit)
        } else {                               // 265
      ...
      // the first packet (fragment).  Now, send the next fragment.  Note that
      // we add "NAL header" and "FU header" bytes to the front.  (We reuse
      // these bytes that we already sent for the first fragment, but clear the
      // S bit, and add the E bit if this is the last fragment.)
...
void H264or5Fragmenter::doStopGettingFrames() {
  // Make sure that we don't have any stale data fragments lying around, should
  // we later resume:
  reset();
