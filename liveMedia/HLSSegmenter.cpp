...
HLSSegmenter::HLSSegmenter(UsageEnvironment &env, unsigned segmentationDuration,
                           ...
                           onEndOfSegmentFunc *onEndOfSegmentFunc,
                           void *onEndOfSegmentClientData)
    : MediaSink(env), fSegmentationDuration(segmentationDuration),
      fFileNamePrefix(fileNamePrefix), fOnEndOfSegmentFunc(onEndOfSegmentFunc),
      fOnEndOfSegmentClientData(onEndOfSegmentClientData),
      fHaveConfiguredUpstreamSource(False), fCurrentSegmentCounter(1),
      fOutFid(NULL) {
...
void HLSSegmenter::ourEndOfSegmentHandler(double segmentDuration) {
  ...
  if (fOnEndOfSegmentFunc != NULL) {
    (*fOnEndOfSegmentFunc)(fOnEndOfSegmentClientData, fOutputSegmentFileName,
                           segmentDuration);
  }
...
void HLSSegmenter::afterGettingFrame(unsigned frameSize,
  ...
  if (numTruncatedBytes > 0) { // Shouldn't happen
    fprintf(
        stderr,
        "HLSSegmenter::afterGettingFrame(frameSize %d, numTruncatedBytes %d)\n",
...
Boolean HLSSegmenter::continuePlaying() {
  ...
  if (!fHaveConfiguredUpstreamSource) {
    ...
    // Tell our upstream multiplexor to call our 'end of segment handler' at the
    // end of each timed segment:
