  ...
  if (fLastStreamToken != NULL && fReuseFirstSource) {
    ...
    if (clientRTPPort.num() != 0 ||
      ...
      if (clientRTCPPort.num() == 0) {
        ...
        // number even). (If we're multiplexing RTCP and RTP over the same port
        // number, it can be odd or even.)
        NoReuse dummy(
            envir()); // ensures that we skip over ports that are already in use
    ...
    streamToken = fLastStreamToken = new StreamState(
        *this, serverRTPPort, serverRTCPPort, rtpSink, udpSink, streamBitrate,
...
FramedSource *
OnDemandServerMediaSubsession::getStreamSource(void *streamToken) {
  if (streamToken == NULL)
    return NULL;
...
char const *
OnDemandServerMediaSubsession ::getAuxSDPLine(RTPSink *rtpSink,
...
Groupsock *
OnDemandServerMediaSubsession ::createGroupsock(struct in_addr const &addr,
                                                Port port) {
...
void StreamState ::startPlaying(
  ...
  if (fRTCPInstance != NULL) {
    ...
    // that receivers will (likely) be able to get RTCP-synchronized
