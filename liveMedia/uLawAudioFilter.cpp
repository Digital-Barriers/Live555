...
uLawFromPCMAudioSource *uLawFromPCMAudioSource ::createNew(
  ...
  if (byteOrdering < 0 || byteOrdering > 2) {
    env.setResultMsg(
        "uLawFromPCMAudioSource::createNew(): bad \"byteOrdering\" parameter");
    return NULL;
  }
...
uLawFromPCMAudioSource ::uLawFromPCMAudioSource(UsageEnvironment &env,
                                                ...
                                                int byteOrdering)
    : FramedFilter(env, inputSource), fByteOrdering(byteOrdering),
...
PCMFromuLawAudioSource ::createNew(UsageEnvironment &env,
                                   FramedSource *inputSource) {
...
NetworkFromHostOrder16 ::createNew(UsageEnvironment &env,
                                   FramedSource *inputSource) {
...
HostFromNetworkOrder16 ::createNew(UsageEnvironment &env,
                                   FramedSource *inputSource) {
