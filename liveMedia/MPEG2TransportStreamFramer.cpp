...
MPEG2TransportStreamFramer *
MPEG2TransportStreamFramer ::createNew(UsageEnvironment &env,
...
MPEG2TransportStreamFramer ::MPEG2TransportStreamFramer(
    ...
    : FramedFilter(env, inputSource), fTSPacketCount(0),
      fTSPacketDurationEstimate(0.0), fTSPCRCount(0),
...
Boolean
MPEG2TransportStreamFramer::updateTSPacketDurationEstimate(unsigned char *pkt,
                                                           double timeNow) {
  ...
  if (pidStatus == NULL) {
    ...
    // appeared unusually quickly. (This can produce more accurate estimates for
    // wildly VBR streams.)
    ...
    fprintf(
        stderr,
        "PID 0x%x, PCR 0x%08x+%d:%03x == %f @ %f (diffs %f @ %f), pkt #%lu, "
        "discon %d => this duration %f, new estimate %f, mean PCR period=%f\n",
        pid, pcrBaseHigh, pkt[10] >> 7, pcrExt, clock, timeNow,
        clock - pidStatus->firstClock, timeNow - pidStatus->firstRealTime,
        fTSPacketCount, discontinuity_indicator != 0, durationPerPacket,
