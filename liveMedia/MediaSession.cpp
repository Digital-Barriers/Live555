...
MediaSession::MediaSession(UsageEnvironment &env)
    : Medium(env), fSubsessionsHead(NULL), fSubsessionsTail(NULL),
      fConnectionEndpointName(NULL), fMaxPlayStartTime(0.0f),
      fMaxPlayEndTime(0.0f), fAbsStartTime(NULL), fAbsEndTime(NULL),
      fScale(1.0f), fSpeed(1.0f), fMediaSessionType(NULL), fSessionName(NULL),
      fSessionDescription(NULL), fControlPath(NULL), fMIKEYState(NULL),
      fCrypto(NULL) {
  ...
  while (sdpLine != NULL) {
    ...
    if (subsession->fCodecName == NULL) {
      subsession->fCodecName = lookupPayloadFormat(
          subsession->fRTPPayloadFormat, subsession->fRTPTimestampFrequency,
...
MediaSubsession::MediaSubsession(MediaSession &parent)
    : serverPortNum(0), sink(NULL), miscPtr(NULL), fParent(parent), fNext(NULL),
      fConnectionEndpointName(NULL), fClientPortNum(0), fRTPPayloadFormat(0xFF),
      fSavedSDPLines(NULL), fMediumName(NULL), fCodecName(NULL),
      fProtocolName(NULL), fRTPTimestampFrequency(0),
      fMultiplexRTCPWithRTP(False), fControlPath(NULL), fMIKEYState(NULL),
      fCrypto(NULL), fSourceFilterAddr(parent.sourceFilterAddr()),
      fBandwidth(0), fPlayStartTime(0.0), fPlayEndTime(0.0),
      fAbsStartTime(NULL), fAbsEndTime(NULL), fVideoWidth(0), fVideoHeight(0),
      fVideoFPS(0), fNumChannels(1), fScale(1.0f), fNPT_PTS_Offset(0.0f),
      fAttributeTable(HashTable::create(STRING_HASH_KEYS)), fRTPSocket(NULL),
      fRTCPSocket(NULL), fRTPSource(NULL), fRTCPInstance(NULL),
      fReadSource(NULL), fReceiveRawMP3ADUs(False),
      fReceiveRawJPEGFrames(False), fSessionId(NULL) {
  rtpInfo.seqNum = 0;
  rtpInfo.timestamp = 0;
  ...
  setAttribute(
      ...
      True /*value is hexadecimal*/); // used with "video/H264"
                                      // This won't work for MPEG-4 (unless the
                                      // value is <10), because for MPEG-4, the
                                      // value is assumed to be a decimal
                                      // string, not a hexadecimal string.  NEED
                                      // TO FIX #####
...
Boolean MediaSubsession::initiate(int useSpecialRTPoffset) {
  ...
  do {
    ...
    if (fClientPortNum != 0 &&
      ...
      // numbers. Create sockets until we get a port-number pair (even: RTP;
      // even+1: RTCP). (However, if we're multiplexing RTCP with RTP, then we
      // create only one socket, and the port number can be even or odd.) We
      // need to make sure that we don't keep trying to use the same bad port
      // numbers over and over again, so we store bad sockets in a table, and
      // delete them all when we're done.
    ...
    if (useSRTP) {
      ...
      // wasn't given to us in the SDP description, then create it now:
      ourCrypto = getCrypto();
    ...
    if (fRTPSource != NULL && fRTCPSocket != NULL) {
      ...
      fRTCPInstance = RTCPInstance::createNew(
          ...
          (unsigned char const *)fParent.CNAME(), NULL /* we're a client */,
...
double
MediaSubsession::getNormalPlayTime(struct timeval const &presentationTime) {
  ...
  // using RTCP. If it hasn't, then - as a special case - we need to use the RTP
  // timestamp to compute the NPT.
  if (!rtpSource()->hasBeenSynchronizedUsingRTCP()) {
    ...
    // "presentationTime" parameter will be accurate, and so we should use this
    // to compute the NPT.
    ...
    if (rtpInfo.infoIsNew) {
      ...
      // presentation time since the "rtpInfo" structure was last filled in. Use
      // this "presentationTime" to compute "fNPT_PTS_Offset":
    ...
    if (strcmp(fProtocolName, "UDP") == 0) {
      ...
      if (strcmp(fCodecName, "QCELP") == 0) { // QCELP audio
        ...
        fReadSource = AMRAudioRTPSource::createNew(
            env(), fRTPSocket, fRTPSource, fRTPPayloadFormat,
            False /*isWideband*/, fNumChannels, attrVal_bool("octet-align"),
            attrVal_unsigned("interleaving"), attrVal_bool("robust-sorting"),
            attrVal_bool("crc"));
        // Note that fReadSource will differ from fRTPSource in this case
      } else if (strcmp(fCodecName, "AMR-WB") == 0) { // AMR audio (wideband)
        fReadSource = AMRAudioRTPSource::createNew(
            env(), fRTPSocket, fRTPSource, fRTPPayloadFormat,
            True /*isWideband*/, fNumChannels, attrVal_bool("octet-align"),
            attrVal_unsigned("interleaving"), attrVal_bool("robust-sorting"),
            attrVal_bool("crc"));
        // Note that fReadSource will differ from fRTPSource in this case
        ...
        fReadSource = fRTPSource = MPEG4GenericRTPSource::createNew(
            ...
            attrVal_unsigned("sizelength"), attrVal_unsigned("indexlength"),
      ...
      if (createSimpleRTPSource) {
        ...
        fReadSource = fRTPSource = SimpleRTPSource::createNew(
            env(), fRTPSocket, fRTPPayloadFormat, fRTPTimestampFrequency,
            mimeType, (unsigned)useSpecialRTPoffset, doNormalMBitRule);
        delete[] mimeType;
      }
    }
...
SDPAttribute::SDPAttribute(char const *strValue, Boolean valueIsHexadecimal)
  ...
  if (fStrValue == NULL) {
    ...
    // Try to parse "fStrValueToLower" as an integer.  If we can't, assume an
    // integer value of 0:
