...
MPEG2TransportStreamMultiplexor ::MPEG2TransportStreamMultiplexor(
    ...
    : FramedSource(env), fHaveVideoStreams(True /*by default*/),
      fOutgoingPacketCounter(0), fProgramMapVersion(0xFF),
      fPreviousInputProgramMapVersion(0xFF), fCurrentInputProgramMapVersion(0),
      fPCR_PID(0), fCurrentPID(0), fInputBuffer(NULL), fInputBufferSize(0),
      fInputBufferBytesUsed(0), fIsFirstAdaptationField(True),
      fSegmentationDuration(0), fSegmentationIndication(1),
...
Boolean
MPEG2TransportStreamMultiplexor::isMPEG2TransportStreamMultiplexor() const {
  return True;
}
...
void MPEG2TransportStreamMultiplexor::doGetNextFrame() {
  ...
  if ((fOutgoingPacketCounter % 10) == 0) {
    ...
    // large input frames, occasionally return to the event loop to do this:
  ...
  if (stream_id == 0xBE) { // padding_stream; ignore
    ...
    if (streamType == 0) {
      ...
      if ((stream_id & 0xF0) == 0xE0) { // video
        ...
        streamType = mpegVersion == 1
                         ? 3
                         : mpegVersion == 2 ? 4 : mpegVersion == 3 ? 6 : 0xF;
...
void MPEG2TransportStreamMultiplexor ::deliverDataToClient(
  ...
  if (fMaxSize < TRANSPORT_PACKET_SIZE) {
    ...
    Boolean willAddPCR =
        pid == fPCR_PID && startPositionInBuffer == 0 &&
        ...
        if (willAddPCR) {
          ...
          *header++ = (u_int8_t)fPCR.extension; // low 8 bits of extension
          ...
          if (fSegmentationDuration > 0) {
            // Use the PCR to compute the duration of the segment so far, to
            // check whether segmentation needs to occur now:
