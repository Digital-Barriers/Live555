...
Boolean
RTSPServer::specialClientAccessCheck(int /*clientSocket*/,
...
RTSPServer::~RTSPServer() {
  ...
  cleanup(); // Removes all "ClientSession" and "ClientConnection" objects, and
             // their tables.
...
void RTSPServer ::unnoteTCPStreamingOnSocket(int socketNum,
  ...
  if (sotcp == sotcpHead) {
    ...
    if (sotcpHead == NULL) {
      // There were no more entries on the list.  Remove the original entry from
      // the hash table:
...
RTSPServer::RTSPClientConnection ::RTSPClientConnection(
    ...
    : GenericMediaServer::ClientConnection(ourServer, clientSocket, clientAddr),
      ...
      fClientOutputSocket(fOurSocket), fIsActive(True), fRecursionCount(0),
...
void RTSPServer::RTSPClientConnection ::handleCmd_SET_PARAMETER(
  ...
  // 'no op', and send back an empty response. (If you want to handle this type
  // of "SET_PARAMETER" differently, you can do so by defining a subclass of
  // "RTSPServer" and "RTSPServer::RTSPClientConnection", and then reimplement
  // this virtual function in your subclass.)
...
Boolean RTSPServer::RTSPClientConnection ::handleHTTPCmd_TunnelingPOST(
  ...
  RTSPServer::RTSPClientConnection *prevClientConnection =
      (RTSPServer::RTSPClientConnection
           *)(fOurRTSPServer.fClientConnectionsForHTTPTunneling->Lookup(
          sessionCookie));
  if (prevClientConnection == NULL || prevClientConnection == this) {
    // Either there was no previous HTTP "GET" request, or it was on the same
    // connection; treat this "POST" request as bad:
...
void RTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead) {
  ...
  do {
...
#ifdef DEBUG
      fprintf(stderr,
              "RTSPClientConnection[%p]::handleRequestBytes() read %d new "
              "bytes (of %d); terminating connection!\n",
    ...
    if (fClientOutputSocket != fClientInputSocket && numBytesRemaining == 0) {
      ...
      // have been Base64-encoded. We therefore Base64-decode as much of this
      // new data as we can (i.e., up to a multiple of 4 bytes).
    ...
    Boolean parseSucceeded = parseRTSPRequestString(
        ...
        sizeof urlSuffix, cseq, sizeof cseq, sessionIdStr, sizeof sessionIdStr,
        contentLength);
    fLastCRLF[2] = '\r'; // restore its value
    // Check first for a bogus "Content-Length" value that would cause a pointer
    // wraparound:
    if (tmpPtr + 2 + contentLength < tmpPtr + 2) {
#ifdef DEBUG
      fprintf(stderr,
              "parseRTSPRequestString() returned a bogus \"Content-Length:\" "
              "value: 0x%x (%d)\n",
    ...
    if (parseSucceeded) {
      ...
      // session that's current ongoing, then use this command to indicate
      // 'liveness' on that client session:
      ...
      if (strcmp(cmdName, "OPTIONS") == 0) {
        ...
        if (!requestIncludedSessionId) {
          ...
          // But first, make sure that we're authenticated to perform this
          // command:
          char urlTotalSuffix[2 * RTSP_PARAM_STRING_MAX];
          // enough space for urlPreSuffix/urlSuffix'\0'
          urlTotalSuffix[0] = '\0';
        ...
        // needs the entire URL, we re-parse the command to get it:
      ...
      *fLastCRLF = '\0'; // temporarily, for parsing
      parseSucceeded = parseHTTPRequestString(
          ...
          sessionCookie, sizeof sessionCookie, acceptStr, sizeof acceptStr);
      *fLastCRLF = '\r';
      if (parseSucceeded) {
#ifdef DEBUG
        fprintf(stderr,
                "parseHTTPRequestString() succeeded, returning cmdName \"%s\", "
                "urlSuffix \"%s\", sessionCookie \"%s\", acceptStr \"%s\"\n",
        ...
        if (strcmp(cmdName, "OPTIONS") == 0) {
          ...
          // application/x-rtsp-tunnelled" header, then this is a bad tunneling
          // request.  Otherwise, assume that it's an attempt to access the
          // stream via HTTP.
          ...
          // command - i.e., the first Base64-encoded RTSP command. Check for
          // this, and handle it if it exists:
    ...
    if (playAfterSetup) {
      ...
      clientSession->handleCmd_withinSession(
          this, "PLAY", urlPreSuffix, urlSuffix, (char const *)fRequestBuffer);
    }

    // Check whether there are extra bytes remaining in the buffer, after the
    // end of the request (a rare case). If so, move them to the front of our
    // buffer, and keep processing it, because it might be a following,
    // pipelined request.
  ...
  if (!fIsActive) {
    ...
    // reenter the event loop and get called recursively while handling a
    // command (e.g., while handling a "DESCRIBE", to get a SDP description). In
    // such a case we don't want to actually delete ourself until we leave the
    // outermost call.
  }
}
...
static Boolean parseAuthorizationHeader(char const *buf, char const *&username,
  ...
  do {
    ...
    SKIP_WHITESPACE;
    for (p = parameter; *fields != '\0' && *fields != ' ' && *fields != '\t' &&
                        *fields != '=';)
      *p++ = *fields++;
    SKIP_WHITESPACE;
    if (*fields++ != '=')
...
Boolean RTSPServer::RTSPClientConnection ::authenticationOK(
  ...
  if (!fOurRTSPServer.specialClientAccessCheck(fClientInputSocket, fClientAddr,
                                               urlSuffix)) {
  ...
  do {
    ...
    // and response string:
    if (!parseAuthorizationHeader(fullRequestStr, username, realm, nonce, uri,
                                  response) ||
  ...
  if (success) {
    ...
    if (!fOurRTSPServer.specialClientUserAccessCheck(
      ...
      // user is valid, even though the server has decided that they should not
      // have access.
      setRTSPResponse("401 Unauthorized");
...
void RTSPServer::RTSPClientConnection ::changeClientInputSocket(
    int newSocketNum, unsigned char const *extraData, unsigned extraDataSize) {
...
void RTSPServer::RTSPClientSession::deleteStreamByTrack(unsigned trackNum) {
  ...
  // Optimization: If all subsessions have now been deleted, then we can delete
  // ourself now:
...
static void parseTransportHeader(char const *buf, StreamingMode &streamingMode,
  ...
  while (sscanf(fields, "%[^;\r\n]", field) == 1) {
    if (strcmp(field, "RTP/AVP/TCP") == 0) {
      ...
      clientRTPPortNum = p1;
      clientRTCPPortNum =
          streamingMode == RAW_UDP ? 0 : p2; // ignore the second port number if
...
void RTSPServer::RTSPClientSession ::handleCmd_SETUP(
  ...
  // "urlSuffix" should be the subsession (track) name. However (being "liberal
  // in what we accept"), we also handle 'aggregate' SETUP requests (i.e.,
  // without a track name), in the special case where we have only a single
  // track.  I.e., in this case, we also handle:
  ...
  do {
    ...
    if (fStreamStates == NULL) {
      ...
      for (unsigned i = 0; i < fNumStreamStates; ++i) {
        ...
        fStreamStates[i].streamToken =
            NULL; // for now; it may be changed by the "getStreamParameters()"
                  // call that comes later
      }
    }
    ...
    if ((streamingMode == RTP_TCP && rtpChannelId == 0xFF) ||
      ...
      //     (QuickTime Player sometimes does this.), or 2/ TCP streaming was
      //     not requested, but we're doing RTSP-over-HTTP tunneling (which
    ...
    // request. This isn't legal, but some clients do this to combine "SETUP"
    // and "PLAY":
    ...
    if (clientsDestinationAddressStr != NULL) {
      ...
      // denial-of-service attacks, so don't enable this code unless you're sure
      // that clients are trusted.
      ...
      case RTP_TCP: {
        if (!fOurRTSPServer.fAllowStreamingRTPOverTCP) {
          ...
          snprintf((char *)ourClientConnection->fResponseBuffer,
                   ...
                   rtcpChannelId, fOurSessionId, timeoutParameterString);
        }
        break;
      }
      case RAW_UDP: {
...
void RTSPServer::RTSPClientSession ::handleCmd_TEARDOWN(
  ...
  // we can reclaim our object now. (Without this optimization, however, this
  // object would still get reclaimed later, as a result of a 'liveness'
  // timeout.)
  Boolean noSubsessionsRemain = True;
  for (i = 0; i < fNumStreamStates; ++i) {
    if (fStreamStates[i].subsession != NULL) {
...
void RTSPServer::RTSPClientSession ::handleCmd_PLAY(
  ...
  if (sawRangeHeader && absStart == NULL /*not seeking by 'absolute' time*/) {
    ...
    // header) have sane values, before we send back our own "Range:" header in
    // our response:
    if (rangeStart < 0.0)
      rangeStart = 0.0;
  ...
  // streaming. (However, we don't do this if the "PLAY" request was for just a
  // single subsession of a multiple-subsession stream; for such streams,
      ...
      if (fStreamStates[i].subsession != NULL) {
        ...
        if (absStart != NULL) {
          ...
          if (!sawRangeHeader || startTimeIsNow) {
            ...
            // seek (to get our NPT, and to specify when to end streaming):
            ...
            fStreamStates[i].subsession->seekStream(
                fOurSessionId, fStreamStates[i].streamToken, rangeStart,
...
void RTSPServer::RTSPClientSession ::handleCmd_GET_PARAMETER(
  ...
  // back a dummy response. (If you want to handle "GET_PARAMETER" properly, you
  // can do so by defining a subclass of "RTSPServer" and
  // "RTSPServer::RTSPClientSession", and then reimplement this virtual function
  // in your subclass.)
  setRTSPResponse(ourClientConnection, "200 OK", fOurSessionId,
