...
StreamParser::StreamParser(FramedSource *inputSource,
    ...
    : fInputSource(inputSource), fClientOnInputCloseFunc(onInputCloseFunc),
      ...
      fClientContinueClientData(clientContinueClientData), fSavedParserIndex(0),
      fSavedRemainingUnparsedBits(0), fCurParserIndex(0),
      fRemainingUnparsedBits(0), fTotNumValidBytes(0), fHaveSeenEOF(False) {
...
void StreamParser::ensureValidBytes1(unsigned numBytesNeeded) {
  ...
  if (fCurParserIndex + numBytesNeeded > BANK_SIZE) {
    ...
    fInputSource->envir() << "StreamParser internal error (" << fCurParserIndex
...
void StreamParser::onInputClosure1() {
  if (!fHaveSeenEOF) {
    ...
    // parsing, as if we'd just read 0 bytes of data. This allows the parser to
    // re-parse any remaining unparsed data (perhaps while testing for EOF at
    // the end):
    fHaveSeenEOF = True;
