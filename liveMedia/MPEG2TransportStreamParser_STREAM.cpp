...
Boolean MPEG2TransportStreamParser ::processStreamPacket(
  ...
  do {
    ...
    if (!streamSource->isCurrentlyAwaitingData()) {
      // Wait until the source next gets read from.  (The parsing will continue
      // then.)
      return False;
    }
...
unsigned MPEG2TransportStreamParser ::parsePESHeader(PIDState_STREAM *pidState,
  ...
  do {
...
#ifdef DEBUG_ERRORS
      fprintf(stderr,
              "MPEG2TransportStreamParser::parsePESHeader(0x%02x, %d): Bad "
              "start code: 0x%06x\n",
    ...
    if (!isSpecialStreamId[stream_id]) {
      ...
      // Make sure that the number of header bytes parsed is consistent with
      // "PES_header_data_length" (and skip over any remasining 'stuffing'
      // bytes):
      if (curOffset() - startPos > 9 + PES_header_data_length) {
#ifdef DEBUG_ERRORS
        fprintf(stderr,
                "MPEG2TransportStreamParser::parsePESHeader(0x%02x, %d): "
                "Error: Parsed %d PES header bytes; expected %d (based on "
    ...
    if (PESHeaderSize > numDataBytes) {
      ...
      fprintf(
          stderr,
          "MPEG2TransportStreamParser::parsePESHeader(0x%02x, %d): Error: PES "
          "header size %d is larger than the number of bytes available (%d)\n",
...
PIDState_STREAM::PIDState_STREAM(MPEG2TransportStreamParser &parser,
                                 ...
                                 u_int8_t streamType)
    : PIDState(parser, pid, STREAM), program_number(programNumber),
      stream_type(streamType), lastSeenPTS(0.0) {
  // Create the 'source' and 'sink' objects for this track, and 'start playing'
  // them:
