...
TCPStreamSink::TCPStreamSink(UsageEnvironment &env, int socketNum)
    : MediaSink(env), fUnwrittenBytesStart(0), fUnwrittenBytesEnd(0),
...
void TCPStreamSink::processBuffer() {
  ...
  if (fOutputSocketIsWritable && numUnwrittenBytes() > 0) {
    ...
    if (numBytesWritten < (int)numUnwrittenBytes()) {
      ...
      if (envir().getErrno() !=
          EPIPE) { // on this error, the socket might still be writable, but no
                   // longer usable
...
void TCPStreamSink::afterGettingFrame(void *clientData, unsigned frameSize,
