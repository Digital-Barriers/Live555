...
private:
  ...
  MediaSubsession
      &fClientMediaSubsession; // the 'client' media subsession object that
                               // corresponds to this 'server' media subsession
  char const
      *fCodecName; // copied from "fClientMediaSubsession" once it's been set up
...
ProxyServerMediaSession ::ProxyServerMediaSession(
    ...
    : ServerMediaSession(env, streamName, NULL, NULL, False, NULL),
      describeCompletedFlag(0), fOurMediaServer(ourMediaServer),
      fClientMediaSession(NULL), fVerbosityLevel(verbosityLevel),
      ...
      fTranscodingTable(transcodingTable), fInitialPortNum(initialPortNum),
...
ProxyRTSPClient::ProxyRTSPClient(ProxyServerMediaSession &ourServerMediaSession,
    ...
    : RTSPClient(ourServerMediaSession.envir(), rtspURL, verbosityLevel,
      ...
      fStreamRTPOverTCP(tunnelOverHTTPPortNum != 0), fSetupQueueHead(NULL),
      fSetupQueueTail(NULL), fNumSetupsDone(0), fNextDESCRIBEDelay(1),
      fServerSupportsGetParameter(False), fLastCommandWasPLAY(False),
      fDoneDESCRIBE(False), fLivenessCommandTask(NULL),
      fDESCRIBECommandTask(NULL), fSubsessionTimerTask(NULL), fResetTask(NULL) {
...
void ProxyRTSPClient::continueAfterDESCRIBE(char const *sdpDescription) {
  if (sdpDescription != NULL) {
    ...
    // "SETUP"/"PLAY" - which doesn't occur until the first time that a client
    // requests the stream. To prevent the proxied connection (between us and
    // the downstream server) from timing out, we send periodic 'liveness'
    // ("OPTIONS" or "GET_PARAMETER") commands.  (The usual RTCP liveness
    // mechanism wouldn't work here, because RTCP packets don't get sent until
    // after the "PLAY" command.)
    scheduleLivenessCommand();
  } else {
    // The "DESCRIBE" command failed, most likely because the server or the
    // stream is not yet running. Reschedule another "DESCRIBE" command to take
    // place later:
...
void ProxyRTSPClient::continueAfterLivenessCommand(
  ...
  if (resultCode != 0) {
    ...
    // restarting the stream. Then continue by sending more "DESCRIBE" commands,
    // to try to restore the stream.
    ...
    if (resultCode < 0) {
      ...
      // server (otherwise "resultCode" would have been > 0). This suggests that
      // the RTSP connection itself has failed.  Print this error code, in case
      // it's useful for debugging:
      if (fVerbosityLevel > 0) {
        envir() << *this
                << ": lost connection to server ('errno': " << -resultCode
                << ").  Scheduling reset...\n";
      }
    }
...
void ProxyRTSPClient::continueAfterSETUP(int resultCode) {
  if (resultCode != 0) {
    // The "SETUP" command failed, so arrange to reset the state. (We don't do
    // this now, because it deletes the "ProxyServerMediaSubsession", and we
  ...
  if (fSetupQueueHead != NULL) {
    ...
    // to do a "SETUP". "SETUP" the first of these now:
    ...
    if (fNumSetupsDone >= smss->fParentSession->numSubsessions()) {
      ...
      // "Range:" header, in case we'd already done a "PLAY" before (as a result
      // of a 'subsession timeout' (note below))
      fLastCommandWasPLAY = True;
    } else {
      // Some of this session's subsessions (i.e., 'tracks') remain to be
      // "SETUP".  They might get "SETUP" very soon, but it's also possible - if
      // the remote client chose to play only some of the session's tracks -
      // that they might not. To allow for this possibility, we set a timer.  If
      // the timer expires without the remaining subsessions getting "SETUP",
...
void ProxyRTSPClient::continueAfterPLAY(int resultCode) {
  if (resultCode != 0) {
    // The "PLAY" command failed, so arrange to reset the state. (We don't do
    // this now, because it deletes the "ProxyServerMediaSubsession", and we
    // can't do that during
    // "ProxyServerMediaSubsession::createNewStreamSource()".)
    scheduleReset();
...
void ProxyRTSPClient::doReset() {
  ...
  setBaseURL(
      fOurURL); // because we'll be sending an initial "DESCRIBE" all over again
  sendDESCRIBE();
}
...
void ProxyRTSPClient::scheduleDESCRIBECommand() {
  ...
  if (fVerbosityLevel > 0) {
    envir() << *this << ": RTSP \"DESCRIBE\" command failed; trying again in "
...
ProxyServerMediaSubsession ::ProxyServerMediaSubsession(
    ...
    : OnDemandServerMediaSubsession(mediaSubsession.parentSession().envir(),
      ...
      fCodecName(strDup(mediaSubsession.codecName())), fNext(NULL),
...
FramedSource *
ProxyServerMediaSubsession::createNewStreamSource(unsigned clientSessionId,
                                                  unsigned &estBitrate) {
  ...
  if (fClientMediaSubsession.readSource() == NULL) {
    ...
    if (fClientMediaSubsession.readSource() != NULL) {
      // First, check whether we have defined a 'transcoder' filter to be used
      // with this codec:
      if (sms->fTranscodingTable != NULL) {
      ...
      // 'normalize' their frames' presentation times, before the frames get
      // re-transmitted by our server:
      ...
      // can be fed into a "RTPSink".  Adjust for this now:
      if (strcmp(fCodecName, "H264") == 0) {
        ...
        fClientMediaSubsession.addFilter(DVVideoStreamFramer ::createNew(
    ...
    if (fClientMediaSubsession.rtcpInstance() != NULL) {
      fClientMediaSubsession.rtcpInstance()->setByeHandler(subsessionByeHandler,
                                                           this);
    }
  }
  ...
  if (clientSessionId != 0) {
    ...
    if (!fHaveSetupStream) {
      ...
      // to the proxied server, to start streaming: (Before sending "SETUP",
      // enqueue ourselves on the "RTSPClient"s 'SETUP queue', so we'll be able
      // to get the correct
      ...
      if (queueWasEmpty) {
        ...
        // Add ourself to the "RTSPClient"s 'SETUP queue' (if we're not already
        // on it):
      ...
      // Hack: If there's already a pending "SETUP" request, don't send this
      // track's "SETUP" right away, because the server might not properly
      // handle 'pipelined' requests.  Instead, wait until after previous
      // "SETUP" responses come back.
      if (queueWasEmpty) {
        proxyRTSPClient->sendSetupCommand(
            fClientMediaSubsession, ::continueAfterSETUP, False,
            proxyRTSPClient->fStreamRTPOverTCP, False, proxyRTSPClient->auth());
        ++proxyRTSPClient->fNumSetupsDone;
        fHaveSetupStream = True;
      }
    } else {
      // This is a "SETUP" from a new client.  We know that there are no other
      // currently active clients (otherwise we wouldn't have been called here),
      // so we know that the substream was previously "PAUSE"d.  Send "PLAY"
      // downstream once again, to resume the stream:
      if (!proxyRTSPClient
               ->fLastCommandWasPLAY) { // so that we send only one "PLAY"; not
                                        // one for each subsession
...
void ProxyServerMediaSubsession::closeStreamSource(FramedSource *inputSource) {
  ...
  // how many downstream clients are proxying it), we don't close the input
  // source here.  (Instead, we wait until *this* object gets deleted.) However,
  // because (as evidenced by this function having been called) we no longer
  // have any clients accessing the stream, then we "PAUSE" the downstream
  // proxied stream, until a new client arrives:
  if (fHaveSetupStream) {
    ProxyServerMediaSession *const sms =
        (ProxyServerMediaSession *)fParentSession;
    ProxyRTSPClient *const proxyRTSPClient = sms->fProxyRTSPClient;
    if (proxyRTSPClient
            ->fLastCommandWasPLAY) { // so that we send only one "PAUSE"; not
                                     // one for each subsession
      if (fParentSession->referenceCount() > 1) {
        // There are other client(s) still streaming other subsessions of this
        // stream. Therefore, we don't send a "PAUSE" for the whole stream, but
        // only for the sub-stream:
        ...
        // this stream. Send a "PAUSE" for the whole stream.
...
RTPSink *ProxyServerMediaSubsession ::createNewRTPSink(
  ...
  if (strcmp(fCodecName, "AC3") == 0 || strcmp(fCodecName, "EAC3") == 0) {
    ...
    newSink = SimpleRTPSink::createNew(
        envir(), rtpGroupsock, 26, 90000, "video", "JPEG", 1 /*numChannels*/,
    ...
    // directly into a corresponding "RTPSink" object.
    if (verbosityLevel() > 0) {
  ...
  // input frames have been RTCP-synchronized, we temporarily disable RTCP "SR"
  // reports for this "RTPSink" object:
...
Groupsock *
ProxyServerMediaSubsession::createGroupsock(struct in_addr const &addr,
                                            Port port) {
...
void PresentationTimeSessionNormalizer ::normalizePresentationTime(
  ...
  if (!hasBeenSynced) {
    ...
    // our own receiving code, and thus is already aligned with 'wall-clock'
    // time.  Just copy it 'as is' to "toPT":
    toPT = fromPT;
  } else {
    if (fMasterSSNormalizer == NULL) {
      // Make "ssNormalizer" the 'master' subsession - meaning that its
      // presentation time is adjusted to align with 'wall clock' time, and the
      // presentation times of other subsessions (if any) are adjusted to retain
      // their relative separation with those of the master:
...
PresentationTimeSubsessionNormalizer ::PresentationTimeSubsessionNormalizer(
    ...
    : FramedFilter(parent.envir(), inputSource), fParent(parent),
      fRTPSource(rtpSource), fRTPSink(NULL), fCodecName(codecName),
      fNext(next) {}
