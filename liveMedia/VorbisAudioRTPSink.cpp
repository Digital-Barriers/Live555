...
VorbisAudioRTPSink ::createNew(UsageEnvironment &env, Groupsock *RTPgs,
  ...
  parseVorbisOrTheoraConfigStr(
      configStr, identificationHeader, identificationHeaderSize, commentHeader,
      commentHeaderSize, setupHeader, setupHeaderSize, identField);
...
VorbisAudioRTPSink ::VorbisAudioRTPSink(
  ...
  if (identificationHeaderSize >= 28) {
    ...
    val =
        ((p[3] * 256 + p[2]) * 256 + p[1]) * 256 + p[0]; // i.e., little-endian
...
void VorbisAudioRTPSink ::doSpecialFrameHandling(
  ...
  u_int8_t numPkts =
      F == 0 ? (numFramesUsedSoFar() + 1) : 0; // set to 0 when we're a fragment
...
char *generateVorbisOrTheoraConfigStr(
  ...
  if (identificationHeaderSize > 0) {
    sizeSize[numHeaders++] = identificationHeaderSize < 128
                                 ? 1
  ...
  *p++ = numHeaders - 1; // "n. of headers"
  if (numHeaders > 1) {
    ...
    unsigned length1 = identificationHeaderSize > 0 ? identificationHeaderSize
                                                    : commentHeaderSize;
    if (length1 >= 16384) {
