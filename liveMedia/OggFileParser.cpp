...
OggFileParser::OggFileParser(OggFile &ourFile, FramedSource *inputSource,
    ...
    : StreamParser(inputSource, onEndFunc, onEndClientData, continueParsing,
      ...
      fNumUnfulfilledTracks(0), fPacketSizeTable(NULL), fCurrentTrackNumber(0),
      fSavedPacket(NULL) {
  if (ourDemux == NULL) {
    // Initialization
...
Boolean OggFileParser::parseStartOfFile() {
  ...
  // we have collected all required headers for Vorbis, Theora, or Opus track(s)
  // (if any).
...
u_int8_t OggFileParser::parseInitialPage() {
  ...
  // see whether the track data type is one that we know how to stream:
  OggTrack *track;
  ...
  if (track != NULL) { // sanity check
    ...
    if (track->mimeType != NULL &&
      ...
      for (unsigned j = 0;
           j < fPacketSizeTable->numCompletedPackets && track->weNeedHeaders();
           ++j) {
        ...
        if (headerIsKnown) {
          ...
          fprintf(stderr, "Saved %d-byte %s \"%s\" header\n", packetSize,
                  track->mimeType, headerName[index]);
#endif
          ...
          // over more than one 'page'.  We assume that that won't ever
          // happen...
        }
      }
    }
...
static Boolean parseVorbisSetup_residues(LEBitVector &bv) {
  ...
  for (unsigned i = 0; i < vorbis_residue_count; ++i) {
    ...
    if (vorbis_residue_type > 2) {
      ...
      bv.skipBits(
          24 + 24 +
          24); // "residue_begin", "residue_end", "residue_partition_size"
...
#endif
    bv.skipBits(16 + 16 +
                8); // "vorbis_mode_windowtype", "vorbis_mode_transformtype",
                    // "vorbis_mode_mapping"
  }
...
Boolean OggFileParser::validateHeader(OggTrack *track, u_int8_t const *p,
  ...
  if (strcmp(track->mimeType, "audio/VORBIS") == 0) {
    ...
    if (firstByte == 1) { // "identification" header
      if (headerSize < 30) {
        fprintf(stderr,
                "Vorbis \"identification\" header is too short (%d bytes)\n",
                headerSize);
        return False;
      } else if ((p[29] & 0x1) != 1) {
        fprintf(
            stderr,
            "Vorbis \"identification\" header: 'framing_flag' is not set\n");
        return False;
      }
      ...
      if (vorbis_version != 0) {
        fprintf(stderr,
                "Vorbis \"identification\" header has a bad 'vorbis_version': "
                "0x%08x\n",
                vorbis_version);
        return False;
      }
      ...
      fprintf(
          ...
          blocksize_0, blocksize_1, track->vtoHdrs.uSecsPerPacket[0],
          track->vtoHdrs.uSecsPerPacket[1]);
#endif
      ...
      if (!(blocksize_0 <= blocksize_1 && blocksize_0 >= 64 &&
            blocksize_1 <= 8192)) {
        fprintf(stderr,
                "Invalid Vorbis \"blocksize_0\" (%d) and/or \"blocksize_1\" "
                "(%d)!\n",
      ...
      // Unfortunately these come near the end of the header, so we have to
      // parse lots of other crap first.
      p += 7;
    ...
    if (firstByte == 0x80) { // "identification" header
      if (headerSize < 42) {
        fprintf(stderr,
                "Theora \"identification\" header is too short (%d bytes)\n",
                headerSize);
        return False;
      } else if ((p[41] & 0x7) != 0) {
        fprintf(stderr,
                "Theora \"identification\" header: 'res' bits are non-zero\n");
        return False;
      }
      ...
      fprintf(
          stderr,
          "\tKFGSHIFT %d, Frame rate numerator %d, Frame rate denominator %d\n",
          track->vtoHdrs.KFGSHIFT, FRN, FRD);
#endif
      if (FRN == 0 || FRD == 0) {
        fprintf(stderr,
                "Theora \"identification\" header: Bad FRN and/or FRD values: "
                "%d, %d\n",
                FRN, FRD);
        return False;
      }
...
#ifdef DEBUG
    fprintf(stderr,
            "\tIgnoring page from unread track; skipping %d remaining packet "
            "data bytes\n",
            fPacketSizeTable->totSizes);
#endif
    skipBytes(fPacketSizeTable->totSizes);
...
Boolean OggFileParser::deliverPacketWithinPage() {
  ...
  if (strcmp(track->mimeType, "audio/VORBIS") == 0) {
    ...
    if (firstByte == 0x4F /*'O'*/ &&
      ...
      // (if necessary) parse the second byte to figure out how many frames are
      // in this packet:
