...
unsigned char *
MPEG4VideoStreamFramer ::getConfigBytes(unsigned &numBytes) const {
...
MPEG4VideoStreamFramer::MPEG4VideoStreamFramer(UsageEnvironment &env,
    ...
    : MPEGVideoStreamFramer(env, inputSource), fProfileAndLevelIndication(0),
      fConfigBytes(NULL), fNumConfigBytes(0), fNewConfigBytes(NULL),
...
MPEG4VideoStreamParser ::MPEG4VideoStreamParser(
    ...
    : MPEGVideoStreamParser(usingSource, inputSource),
      ...
      vop_time_increment_resolution(0), fNumVTIRBits(0), fixed_vop_rate(0),
      fixed_vop_time_increment(0), fSecondsSinceLastTimeCode(0),
      fTotalTicksSinceLastTimeCode(0), fPrevNewTotalTicks(0),
      fPrevPictureCountDelta(1), fJustSawTimeCode(False) {}
...
void MPEG4VideoStreamParser::flushInput() {
  ...
  if (fCurrentParseState != PARSING_VISUAL_OBJECT_SEQUENCE) {
    setParseState(
        PARSING_VISUAL_OBJECT_SEQUENCE); // later, change to GOV or VOP? #####
  }
}
...
unsigned
MPEG4VideoStreamParser ::parseVisualObjectSequence(Boolean haveSeenStartCode) {
#ifdef DEBUG
...
unsigned MPEG4VideoStreamParser::parseVideoObjectLayer() {
  ...
  if (!isVideoObjectLayerStartCode(next4Bytes)) {
    usingSource()->envir()
        << "MPEG4VideoStreamParser::parseVideoObjectLayer(): This appears to "
           "be a 'short video header', which we currently don't support\n";
  }
...
unsigned MPEG4VideoStreamParser::parseGroupOfVideoObjectPlane() {
  ...
  // we don't pass "fTotalTicksSinceLastTimeCode" as the "picturesSinceLastGOP"
  // parameter.
  fSecondsSinceLastTimeCode = 0;
  if (fixed_vop_rate)
    fTotalTicksSinceLastTimeCode = 0;
...
unsigned MPEG4VideoStreamParser::parseVideoObjectPlane() {
  ...
  if ((mask >> (fNumVTIRBits - 1)) == 0) {
    usingSource()->envir()
        << "MPEG4VideoStreamParser::parseVideoObjectPlane(): 32-bits are not "
           "enough to get \"vop_time_increment\"!\n";
  }
  ...
  fprintf(
      stderr,
      "vop_coding_type: %d(%c), modulo_time_base: %d, vop_time_increment: %d\n",
  ...
  if (fixed_vop_time_increment > 0) {
    ...
    if (newTotalTicks == fPrevNewTotalTicks && fPrevNewTotalTicks > 0) {
      ...
      if (newTotalTicks < fPrevNewTotalTicks && vop_coding_type != 2 /*B*/
        ...
        // Overcome this by pretending that "vop_time_increment" *did* wrap
        // around:
#ifdef DEBUG
        fprintf(stderr,
