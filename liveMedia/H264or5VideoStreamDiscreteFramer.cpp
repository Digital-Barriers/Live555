...
void H264or5VideoStreamDiscreteFramer ::afterGettingFrame1(
  ...
  // begin with a 0x00000001 or 0x000001 'start code'.  (Those start codes
  // should only be in byte-stream data; *not* data that consists of discrete
  // NAL units.) Once again, to be clear: The NAL units that you feed to a
...
Boolean H264or5VideoStreamDiscreteFramer::nalUnitEndsAccessUnit(
  ...
  // assuming that if this NAL unit is a VCL NAL unit, then it ends the current
  // 'access unit'.
  //
