  ...
  do {
    ...
    if (endPos - startPos > numDataBytes) {
      ...
      fprintf(stderr,
              ...
              "section_length %d gives us a total size %d that's too large!\n",
              pidState->PID, pusi, numDataBytes, section_length,
              endPos - startPos);
#endif
      break;
    ...
    if (program_number != pidState->program_number) {
      ...
      fprintf(stderr,
              "MPEG2TransportStreamParser::parsePMT(0x%04x, %d, %d): "
              "program_number %d does not match the value %d that was given to "
              "us in the PAT!\n",
    ...
    fprintf(
        stderr,
        "\t\tversion_number: %d; section_number: %d; last_section_number: %d\n",
    ...
    if (endOfDescriptors + 4 /*CRC*/ - startPos > numDataBytes) {
      ...
      fprintf(
          stderr,
          "MPEG2TransportStreamParser::parsePMT(0x%04x, %d, %d): "
          "program_info_length %d gives us a total size %d that's too large!\n",
    ...
    while (curOffset() <= endPos - 4 /*for CRC*/ - 5 /*for mapping fields*/) {
      ...
      fprintf(stderr,
              ...
              "ES_info_length: %d\n",
              stream_type, streamTypeDesc == NULL ? "???" : streamTypeDesc,
      ...
      if (endOfDescriptors + 4 /*CRC*/ - startPos > numDataBytes) {
        ...
        fprintf(
            stderr,
            "MPEG2TransportStreamParser::parsePMT(0x%04x, %d, %d): "
            "ES_info_length %d gives us a total size %d that's too large!\n",
...
void MPEG2TransportStreamParser::parseStreamDescriptors(
  ...
  while (numDescriptorBytes >=
...
#ifdef DEBUG_ERRORS
      fprintf(stderr,
              "MPEG2TransportStreamParser::parseStreamDescriptors() error: Saw "
              "descriptor_length %d > remaining bytes %d\n",
    ...
    switch (descriptor_tag) {
      // Note: These are the tags that we've seen to date.  Add more when we see
      // more.
    case 0x02: {
      pDesc("video");
...
#ifdef DEBUG_CONTENTS
      fprintf(stderr,
              "\t\t\t\tflags: 0x%02x (frame_rate_code 0x%1x; MPEG_1_only_flag "
              "%d)\n",
      ...
      if (MPEG_1_only_flag == 0) {
        ...
        flags = get1Byte();
        fprintf(stderr,
                "\t\t\t\tprofile_and_level_indication 0x%02x; flags 0x%02x "
                "(chroma_format 0x%1x)\n",
...
#ifdef DEBUG_CONTENTS
        fprintf(stderr,
                "\t\t\t\tISO_639_language_code: %c%c%c; audio_type: 0x%02x\n",
    ...
    case 0x0b: {
      ...
      for (unsigned i = 0; i < clock_accuracy_exponent; ++i)
        ppm /= 10.0;
      fprintf(stderr,
              "\t\t\t\texternal_clock: %d; clock_accuracy int: %d, exp: %d -> "
              "%f ppm\n",
    ...
    case 0x1d: {
      ...
      // Note: We don't know how to parse this.  (Where's a document that
      // describes this?)
      skipBytes(descriptor_length);
      numDescriptorBytes -= descriptor_length;
      parsedDescriptor = True;
    ...
    case 0x28: {
      ...
      fprintf(stderr,
              "\t\t\t\tprofile_idc: 0x%02x, flags1: 0x%02x, level_idc: 0x%02x, "
              "flags2: 0x%02x\n",
...
#ifdef DEBUG_CONTENTS
        fprintf(stderr, "\t\t\t\tISO_639_language_code: %c%c%c", get1Byte(),
                get1Byte(), get1Byte());
    ...
    case 0x59: {
      ...
      for (unsigned i = 0; i < descriptor_length / 8; ++i) {
        ...
        fprintf(stderr, "\t\t\t\tISO_639_language_code: %c%c%c", get1Byte(),
                get1Byte(), get1Byte());
        fprintf(stderr,
                "; subtitling_type: 0x%02x; composition_page_id: 0x%04x; "
                "ancillary_page_id: 0x%04x\n",
...
#ifdef DEBUG_CONTENTS
        fprintf(stderr,
                "\t\t\t\tapplication_type: 0x%04x; AIT_version_number: %d\n",
...
#ifdef DEBUG_CONTENTS
      u_int8_t flags = get1Byte();
      fprintf(stderr, "\t\t\t\tsample_rate_code: %d; bsid: 0x%02x", flags >> 5,
              flags & 0x1F);
      flags = get1Byte();
      fprintf(stderr, "; bit_rate_code: %d; surround_mode: %d", flags >> 2,
              flags & 0x03);
      flags = get1Byte();
      fprintf(stderr, "; bsmod: %d; num_channels: %d; full_svc: %d", flags >> 5,
              (flags & 0x1E) >> 1, (flags & 0x01));
      if (descriptor_length > 3) {
        fprintf(stderr, "; extra bytes: ");
    ...
    case 0x86: {
      ...
      for (unsigned i = 0; i < number_of_services; ++i) {
        fprintf(stderr, "\t\t\t\t\tlanguage: %c%c%c", get1Byte(), get1Byte(),
                get1Byte());
