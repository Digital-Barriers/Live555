...
H264VideoRTPSink ::createNew(UsageEnvironment &env, Groupsock *RTPgs,
  ...
  H264VideoRTPSink *result = new H264VideoRTPSink(env, RTPgs, rtpPayloadFormat,
...
char const *H264VideoRTPSink::auxSDPLine() {
  ...
  // them), otherwise parameters from our framer source (in case they've changed
  // since the last time that we were called):
  ...
  if (sps == NULL || pps == NULL) {
    ...
    framerSource->getVPSandSPSandPPS(vpsDummy, vpsDummySize, sps, spsSize, pps,
                                     ppsSize);
    if (sps == NULL || pps == NULL)
