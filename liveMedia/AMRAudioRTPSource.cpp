...
AMRAudioSource *AMRAudioRTPSource::createNew(
  ...
  resultRTPSource = rawRTPSource =
      RawAMRRTPSource::createNew(env, RTPgs, rtpPayloadFormat, isWideband,
...
RawAMRRTPSource ::RawAMRRTPSource(UsageEnvironment &env, Groupsock *RTPgs,
    ...
    : MultiFramedRTPSource(env, RTPgs, rtpPayloadFormat,
      ...
      fIsInterleaved(isInterleaved), fCRCsArePresent(CRCsArePresent), fILL(0),
      fILP(0), fTOCSize(0), fTOC(NULL), fFrameIndex(0), fIsSynchronized(False) {
}
...
Boolean
RawAMRRTPSource ::processSpecialHeader(BufferedPacket *packet,
                                       unsigned &resultSpecialHeaderSize) {
...
BufferedPacket *
AMRBufferedPacketFactory ::createNewPacket(MultiFramedRTPSource *ourSource) {
...
AMRDeinterleaver::AMRDeinterleaver(UsageEnvironment &env, Boolean isWideband,
                                   ...
                                   unsigned maxInterleaveGroupSize,
                                   RawAMRRTPSource *inputSource)
    : AMRAudioSource(env, isWideband, numChannels), fInputSource(inputSource),
      fNeedAFrame(False) {
...
AMRDeinterleavingBuffer ::AMRDeinterleavingBuffer(
    ...
    : fNumChannels(numChannels),
      fMaxInterleaveGroupSize(maxInterleaveGroupSize), fIncomingBankId(0),
      fIncomingBinMax(0), fOutgoingBinMax(0), fNextOutgoingBin(0),
...
Boolean AMRDeinterleavingBuffer ::retrieveFrame(
  ...
  if (outBin.fIsSynchronized) {
    ...
    // at least a complete interleave cycle of synchronized frames.  This
    // ensures that the receiver will be getting all synchronized frames from
    // now on.
...
#ifdef DEBUG
      fprintf(stderr,
              "\tWarning: Unpacking frame %d of %d: want %d bits, but only %d "
              "are available!\n",
