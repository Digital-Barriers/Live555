...
RTSPServerSupportingHTTPStreaming ::RTSPServerSupportingHTTPStreaming(
    UsageEnvironment &env, int ourSocket, Port rtspPort,
    UserAuthenticationDatabase *authDatabase, unsigned reclamationTestSeconds)
    : RTSPServer(env, ourSocket, rtspPort, authDatabase,
...
RTSPServerSupportingHTTPStreaming::
    ...
    : RTSPClientConnection(ourServer, clientSocket, clientAddr),
      fClientSessionId(0), fStreamSource(NULL), fPlaylistSource(NULL),
      fTCPSink(NULL) {}
...
void RTSPServerSupportingHTTPStreaming::
    RTSPClientConnectionSupportingHTTPStreaming ::handleHTTPCmd_StreamingGET(
  ...
  // and send the specified segment.  Otherwise, construct and send a playlist
  // that consists of segments from the specified file.
  do {
    ...
    do {
      ...
      // defined way to multiplex more than one subsession). Therefore, use the
      // first (and presumed only) substream:
      ...
      // we're not actually streaming via RTP/RTCP, most of the parameters to
      // the call are dummy.)
      ++fClientSessionId;
      ...
      subsession->getStreamParameters(
          fClientSessionId, 0, clientRTPPort, clientRTCPPort, -1, 0, 0,
  ...
  // "?segment=<offset-in-seconds>,<duration-in-seconds>". Construct and send a
  // playlist that describes segments from the specified file.
  ...
  unsigned const maxIntLen =
      10; // >= the maximum possible strlen() of an integer in the playlist
...
void RTSPServerSupportingHTTPStreaming::
  ...
  RTSPServerSupportingHTTPStreaming::RTSPClientConnectionSupportingHTTPStreaming
      *clientConnection =
          (RTSPServerSupportingHTTPStreaming::
               RTSPClientConnectionSupportingHTTPStreaming *)clientData;
