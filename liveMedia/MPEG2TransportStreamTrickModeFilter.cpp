...
MPEG2TransportStreamTrickModeFilter ::MPEG2TransportStreamTrickModeFilter(
    UsageEnvironment &env, FramedSource *inputSource,
    MPEG2TransportStreamIndexFile *indexFile, int scale)
    : FramedFilter(env, inputSource), fHaveStarted(False),
      fIndexFile(indexFile), fScale(scale), fDirection(1),
      fState(SKIPPING_FRAME), fFrameCount(0), fNextIndexRecordNum(0),
      fNextTSPacketNum(0), fCurrentTSPacketNum((unsigned long)(-1)),
...
Boolean
MPEG2TransportStreamTrickModeFilter::seekTo(unsigned long tsPacketNumber,
