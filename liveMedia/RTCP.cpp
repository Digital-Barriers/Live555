...
RTCPInstance::RTCPInstance(UsageEnvironment &env, Groupsock *RTCPgs,
    ...
    : Medium(env), fRTCPInterface(this, RTCPgs), fTotSessionBW(totSessionBW),
      fSink(sink), fSource(source), fIsSSMTransmitter(isSSMTransmitter),
      fCrypto(crypto), fCNAME(RTCP_SDES_CNAME, cname), fOutgoingReportCount(1),
      fAveRTCPSize(0), fIsInitial(1), fPrevNumMembers(0), fLastSentSize(0),
      fLastReceivedSize(0), fLastReceivedSSRC(0), fTypeOfEvent(EVENT_UNKNOWN),
      fTypeOfPacket(PACKET_UNKNOWN_TYPE), fHaveJustSentPacket(False),
      fLastPacketSentSize(0), fByeHandlerTask(NULL),
      fByeWithReasonHandlerTask(NULL), fByeHandlerClientData(NULL),
      fSRHandlerTask(NULL), fSRHandlerClientData(NULL), fRRHandlerTask(NULL),
      fRRHandlerClientData(NULL), fSpecificRRHandlerTable(NULL),
  ...
  if (fSource != NULL && fSource->RTPgs() == RTCPgs) {
    // We're receiving RTCP reports that are multiplexed with RTP, so ask the
    // RTP source to give them to us:
...
RTCPInstance::~RTCPInstance() {
  ...
  if (fSource != NULL && fSource->RTPgs() == fRTCPInterface.gs()) {
    // We were receiving RTCP reports that were multiplexed with RTP, so tell
    // the RTP source to stop giving them to us:
...
void RTCPInstance::noteArrivingRR(struct sockaddr_in const &fromAddressAndPort,
  ...
  if (fSpecificRRHandlerTable != NULL) {
    ...
    RRHandlerRecord *rrHandler =
        (RRHandlerRecord *)(fSpecificRRHandlerTable->Lookup(fromAddr, (~0),
                                                            fromPort));
    if (rrHandler != NULL) {
...
void RTCPInstance ::unsetSpecificRRHandler(netAddressBits fromAddress,
  ...
  RRHandlerRecord *rrHandler =
      (RRHandlerRecord *)(fSpecificRRHandlerTable->Lookup(fromAddress, (~0),
                                                          fromPort));
  if (rrHandler != NULL) {
  ...
  do {
    if (fNumBytesAlreadyRead >= maxRTCPPacketSize) {
      envir() << "RTCPInstance error: Hit limit when reading incoming packet "
              ...
              << maxRTCPPacketSize
              << ")).  The remote endpoint is using a buggy implementation of "
                 "RTP/RTCP-over-TCP.  Please upgrade it!\n";
      break;
    }
    ...
    Boolean readResult = fRTCPInterface.handleRead(
        &fInBuf[fNumBytesAlreadyRead], maxRTCPPacketSize - fNumBytesAlreadyRead,
        numBytesRead, fromAddress, tcpSocketNum, tcpStreamChannelId,
    ...
    if (fIsSSMTransmitter && !packetWasFromOurHost) {
      ...
      // a SSM transmitter, and SSM receivers send back RTCP "RR" packets via
      // unicast). 'Reflect' the packet by resending it to the multicast group,
      // so that any other receivers can also get to see it.
      ...
      // RTCP packets that come from other processes on the same host as us. The
      // reason for this is that the 'packet size' test above is not 100%
      // reliable; some packets that were truly looped back from us might not be
      // detected as such, and this might lead to infinite forwarding/receiving
      // of some packets.  To avoid this possibility, we reflect only RTCP
      // packets that we know for sure originated elsewhere. (Note, though, that
      // if we ever re-enable the code in "Groupsock::multicastSendOnly()", then
      // we could remove the test for "!packetWasFromOurHost".)
...
void RTCPInstance ::processIncomingReport(
    unsigned packetSize, struct sockaddr_in const &fromAddressAndPort,
    int tcpSocketNum, unsigned char tcpStreamChannelId) {
  do {
    if (fCrypto !=
        NULL) { // The packet is assumed to be SRTCP.  Verify/decrypt it first:
    ...
    char *reason =
        NULL; // by default, unless/until a BYE packet with a 'reason' arrives
    unsigned char *pkt = fInBuf;

#ifdef DEBUG
    fprintf(stderr, "[%p]saw incoming RTCP packet (from ", this);
    if (tcpSocketNum < 0) {
      // Note that "fromAddressAndPort" is valid only if we're receiving over
      // UDP (not over TCP):
    ...
    while (1) {
      ...
      if (reportSenderSSRC == 0x00000001 && pt == RTCP_PT_RR) {
        // Chrome (and Opera) WebRTC receivers have a bug that causes them to
        // always send SSRC 1 in their "RR"s.  To work around this (to help us
        // distinguish between different receivers), we use a fake SSRC in this
        // case consisting of the IP address, XORed with the port number:
        reportSenderSSRC =
          ...
          for (unsigned i = 0; i < rc; ++i) {
            ...
            if (senderSSRC == fSink->SSRC()) {
              ...
              transmissionStats.noteIncomingRR(
                  reportSenderSSRC, fromAddressAndPort, lossStats,
                  highestReceived, jitter, timeLastSR, timeSinceLastSR);
            } else {
              ADVANCE(4 * 5);
            }
          }
      ...
      case RTCP_PT_BYE: {
...
#ifdef DEBUG
            fprintf(stderr,
                    "\nError: The 'reason' length %d is too large (it should "
                    "be <= %d)\n",
        ...
        // this routine. (Note: We don't call it immediately, in case it happens
        // to cause "this" to be deleted.)
      ...
      case RTCP_PT_APP: {
        u_int8_t &subtype =
            rc; // In "APP" packets, the "rc" field gets used as "subtype"
#ifdef DEBUG
        ...
        if (fAppHandlerTask != NULL) {
          (*fAppHandlerTask)(fAppHandlerClientData, subtype, nameBytes, pkt,
                             length);
        }
        subPacketOK = True;
      ...
      case RTCP_PT_SDES: {
#ifdef DEBUG
        // 'Handle' SDES packets only in debugging code, by printing out the
        // 'SDES items':
        fprintf(stderr, "SDES\n");
        ...
        while (length >= 8) { // A valid chunk must be at least 8 bytes long
          ...
          while (itemType != 0) {
            ...
            ADVANCE(1);
            --length;
            // Make sure "itemLen" allows for at least 1 zero byte at the end of
            // the chunk:
          ...
          // Thus, itemType == 0.  This zero 'type' marks the end of the list of
          // SDES items. Skip over remaining zero padding bytes, so that this
          // chunk ends on a 4-byte boundary:
      ...
      case RTCP_PT_PSFB: {
        ...
        // Temporary code to show "Receiver Estimated Maximum Bitrate" (REMB)
        // feedback reports:
        //#####
...
#ifdef DEBUG
      fprintf(stderr,
              "validated RTCP subpacket: rc:%d, pt:%d, bytes remaining:%d, "
              "report sender SSRC:0x%08x\n",
...
Boolean RTCPInstance::addReport(Boolean alwaysAdd) {
  ...
  if (fSink != NULL) {
    if (!alwaysAdd) {
      ...
      // the next outgoing RTP packet has been preset, to ensure that that
      // timestamp gets used for that outgoing packet. (David Bertrand,
      // 2006.07.18)
