...
QuickTimeFileSink::QuickTimeFileSink(
    ...
    : Medium(env), fInputSession(inputSession), fBufferSize(bufferSize),
      fPacketLossCompensate(packetLossCompensate), fSyncStreams(syncStreams),
      fGenerateMP4Format(generateMP4Format), fAreCurrentlyBeingPlayed(False),
      fLargestRTPtimestampFrequency(0), fNumSubsessions(0),
      fNumSyncedSubsessions(0), fHaveCompletedOutputFile(False),
...
Boolean QuickTimeFileSink::continuePlaying() {
  ...
  while ((subsession = iter.next()) != NULL) {
    ...
    subsessionSource->getNextFrame(toPtr, toSize, afterGettingFrame, ioState,
...
void QuickTimeFileSink ::afterGettingFrame(
  ...
  if (numTruncatedBytes > 0) {
    ioState->envir()
        << "QuickTimeFileSink::afterGettingFrame(): The input frame data was "
...
SubsessionIOState::SubsessionIOState(QuickTimeFileSink &sink,
                                     MediaSubsession &subsession)
    : fHintTrackForUs(NULL), fTrackHintedByUs(NULL), fOurSink(sink),
      fOurSubsession(subsession), fLastPacketRTPSeqNum(0),
      fHaveBeenSynced(False), fQTTotNumSamples(0), fHeadChunk(NULL),
      fTailChunk(NULL), fNumChunks(0), fHeadSyncFrame(NULL),
      fTailSyncFrame(NULL) {
...
Boolean SubsessionIOState::setQTstate() {
  ...
  char const *noCodecWarning3 =
      "\" track, so we'll insert a dummy \"????\" Media Data Atom instead.  A "
      "separate, codec-specific editing pass will be needed before this track "
      "can be played.\n";
  ...
  do {
    ...
    if (isHintTrack()) {
      ...
      if (strcmp(fOurSubsession.codecName(), "X-QT") == 0 ||
        ...
        // aacPlus).
        unsigned frequencyFromConfig = samplingFrequencyFromAudioSpecificConfig(
...
void SubsessionIOState::useFrameForHinting(unsigned frameSize,
  ...
  if (ppt.tv_sec != 0 || ppt.tv_usec != 0) {
    ...
    if (hackm4a) {
      ...
      // presentation times of the second and subsequent frames will not be
      // accurate. So, use the known "hintSampleDuration" instead:
      ...
      if (fTrackHintedByUs->fQTTimeScale !=
        ...
        unsigned const scalingFactor = fOurSubsession.rtpTimestampFrequency() /
...
ChunkDescriptor ::ChunkDescriptor(int64_t offsetInFile, unsigned size,
    ...
    : fNextChunk(NULL), fOffsetInFile(offsetInFile),
      ...
      fFrameDuration(frameDuration), fPresentationTime(presentationTime) {}
  ...
  unsigned QuickTimeFileSink::addAtom_genericMedia() {
    ...
    unsigned QuickTimeFileSink::addAtom_soundMediaGeneral() {
      ...
      unsigned QuickTimeFileSink::addAtom_Qclp() {
        ...
        // atom, except with a version field of 1:
        int64_t initFilePosn = TellFile64(fOutFid);
        ...
        unsigned QuickTimeFileSink::addAtom_mp4a() {
          ...
          unsigned QuickTimeFileSink::addAtom_rtp() {
            ...
            size +=
                addWord(0x00000001); // Reserved (last 2 bytes) + Data ref index
            ...
            if (fCurrentIOState->fHeadSyncFrame != NULL) {
              ...
              // First, count the number of frames (to use as a sanity check;
              // see below):
              ...
              // First, run through the chunk descriptors, counting up the total
              // number of samples:
              unsigned const samplesPerFrame =
                  fCurrentIOState->fQTSamplesPerFrame;
              ChunkDescriptor *chunk = fCurrentIOState->fHeadChunk;
