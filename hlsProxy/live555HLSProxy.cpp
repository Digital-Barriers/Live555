...
int main(int argc, char **argv) {
  ...
  while (argc > 1) {
    ...
    case 'U': { // specify a username and password to be used to authentication
  ...
  if (createHandlerServerForREGISTERCommand) {
    ...
    if (usernameForREGISTER != NULL) {
      *env << "The '-U <username-for-REGISTER> <password-for-REGISTER>' option "
...
void continueAfterDESCRIBE(RTSPClient *rtspClient, int resultCode,
  ...
  do {
    ...
    if (!subsession->initiate()) {
      *env << *rtspClient << "Failed to initiate the \"" << *subsession
    ...
    rtspClient->sendSetupCommand(*subsession, continueAfterSETUP, False,
                                 streamUsingTCP, False, ourAuthenticator);
    return;
  } while (0);
...
void continueAfterSETUP(RTSPClient *rtspClient, int resultCode,
  ...
  do {
    ...
    *env << *rtspClient << "Set up the \"" << *subsession << "\" subsession\n";
    ...
    subsession->sink = HLSSegmenter::createNew(
        *env, OUR_HLS_SEGMENTATION_DURATION, hlsPrefix, segmentationCallback);

    // Create a 'framer' filter for the input source, to put the stream of NAL
    // units into a form that's usable in output Transport Streams. (Note that
    // we use a *DiscreteFramer*, because the input source is a stream of
    // discrete
    ...
    *env << "Beginning to read...\n";
    ...
    // Finally, send a RTSP "PLAY" command to tell the server to start
    // streaming:
...
public:
  SegmentRecord(char const *segmentFileName, double segmentDuration)
      : fNext(NULL), fSegmentFileName(strDup(segmentFileName)),
...
void segmentationCallback(void * /*clientData*/, char const *segmentFileName,
  ...
  while (totalDuration > OUR_HLS_REWIND_DURATION) {
    ...
    fprintf(stderr,
            "\tDeleting segment \"%s\" (duration: %f seconds) -> %f seconds of "
            "data stored\n",
  ...
  if (isFirstTime) {
    fprintf(stderr,
            "Wrote index file \"%s\"; the stream can now be played from a URL "
            "pointing to this file.\007\n",
            ourM3U8FileName);
    isFirstTime = False;
  }
}
