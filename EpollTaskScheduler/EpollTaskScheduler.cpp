...
EpollTaskScheduler *
EpollTaskScheduler::createNew(unsigned maxSchedulerGranularity) {
...
void EpollTaskScheduler::SingleStep(unsigned maxDelayTime) {
  ...
  const int timeout =
      tv_timeToDelay.tv_sec * 1000 + tv_timeToDelay.tv_usec / 1000;
  epoll_event event;
  ...
  // Also handle any newly-triggered event (Note that we do this *after* calling
  // a socket handler, in case the triggered event handler modifies The set of
  // readable sockets.)
  if (fTriggersAwaitingHandling != 0) {
    if (fTriggersAwaitingHandling == fLastUsedTriggerMask) {
      ...
      // Look for an event trigger that needs handling (making sure that we make
      // forward progress through all possible triggers):
...
void EpollTaskScheduler ::setBackgroundHandling(
...
void EpollTaskScheduler::moveSocketHandling(int oldSocketNum,
                                            int newSocketNum) {
...
EpollTaskScheduler::lookupHandlerDescriptor(int socketNum) const {
